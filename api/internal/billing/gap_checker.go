...
func NewGapChecker(eegRepo *repository.EEGRepository, meterPointRepo *repository.MeterPointRepository, emailLogRepo *repository.EmailLogRepository) *GapChecker {
	return &GapChecker{
		eegRepo:        eegRepo,
...
func (g *GapChecker) sendGapEmail(ctx context.Context, eeg *domain.EEG, gaps []repository.MeterPointGap) {
...
func (g *GapChecker) sendHTMLEmail(ctx context.Context, eeg *domain.EEG, emailType, subject, htmlBody string) {
	...
	if err := invoice.SendLogged(ctx, g.emailLogRepo, cfg, eeg.ID, emailType, eeg.SMTPFrom, subject, nil, nil, []byte(msg.String())); err != nil {
