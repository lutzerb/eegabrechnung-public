...
func (s *Scheduler) checkEEG(ctx context.Context, eeg *domain.EEG, today time.Time) {
	...
	// 4. Data completeness check — every active ZP must have L1/L2 for every 15-min slot
	missingZPs, err := s.readingRepo.MissingIntervals(ctx, eeg.ID, periodStart, periodEnd)
	if err != nil {
	...
	if len(missingZPs) > 0 {
		...
		s.sendWarningEmail(ctx, eeg, periodStart, periodEnd, missingZPs)
		return
	}
	...
	if err != nil {
		if isOverlapError(err) {
			...
			s.sendErrorEmail(ctx, eeg, periodStart, periodEnd, err)
		}
		return
	}
...
func (s *Scheduler) sendSuccessEmail(ctx context.Context, eeg *domain.EEG, runID uuid.UUID, from, to time.Time, invoiceCount int) {
...
func (s *Scheduler) sendWarningEmail(ctx context.Context, eeg *domain.EEG, from, to time.Time, missingZPs []string) {
	if eeg.SMTPHost == "" || eeg.SMTPFrom == "" {
...
func (s *Scheduler) sendErrorEmail(ctx context.Context, eeg *domain.EEG, from, to time.Time, runErr error) {
	if eeg.SMTPHost == "" || eeg.SMTPFrom == "" {
		return
	}
...
func (s *Scheduler) sendHTMLEmail(ctx context.Context, eeg *domain.EEG, emailType, subject, htmlBody string) {
	...
	if err := invoice.SendLogged(ctx, s.emailLogRepo, cfg, eeg.ID, emailType, eeg.SMTPFrom, subject, nil, nil, []byte(msg.String())); err != nil {
