...
func ParseEnergieDaten(path string) ([]domain.EnergyRow, error) {
	...
	//
	// Each value column can be immediately followed by an "MM"-labeled quality column
	// carrying the per-interval quality code (L0/L1/L2/L3) for that value — e.g. column
	// "Gesamtverbrauch..." at index N, its per-interval quality at index N+1 (meter row
	...
	isQualityCol := func(col int) bool {
		...
		return strings.ToUpper(strings.TrimSpace(meterRow[col])) == "MM"
	}
	...
	for col, meter := range colToMeter {
		...
		if mc == nil {
			mc = &meterCols{total: -1, community: -1, self: -1, residual: -1, totalQual: -1, communityQual: -1, selfQual: -1, residualQual: -1}
			meterColMap[meter] = mc
		}
		qualCol := -1
		if isQualityCol(col + 1) {
			qualCol = col + 1
		}
		switch ct {
		case colTypeTotal:
			if mc.total < 0 {
				mc.total = col
				mc.totalQual = qualCol
			}
		case colTypeCommunity:
			if mc.community < 0 {
				mc.community = col
				mc.communityQual = qualCol
			}
		case colTypeSelf:
			if mc.self < 0 {
				mc.self = col
	...
	for i := dataStartIdx; i < len(rows); i++ {
		...
		for meter, mc := range meterColMap {
			...
			// Quality: prefer the "total" column's own quality (Gesamtverbrauch/-erzeugung is
			// the primary billing quantity), falling back to whichever other component column
			// carries one. Empty string if the file has no "MM" quality columns at all (e.g.
			// a manually-built XLSX) — BulkUpsert then defaults it to L0.
			quality := getString(paddedRow, mc.totalQual)
			if quality == "" {
				quality = getString(paddedRow, mc.communityQual)
			}
			if quality == "" {
				quality = getString(paddedRow, mc.selfQual)
			}
			if quality == "" {
				quality = getString(paddedRow, mc.residualQual)
			}
...
func getFloat(row []string, col int) float64 {
	...
	// Format B has "L1" quality suffix — ignore it (the paired "MM" column carries the
	// same code separately and is read via getString/getQualityCol instead).
...
// getString returns the trimmed string value of a quality ("MM") column, or "" if col is
// absent (-1) or out of range.
func getString(row []string, col int) string {
	if col < 0 || col >= len(row) {
		return ""
	}
