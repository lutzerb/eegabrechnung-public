...
func (t *MailTransport) Poll(ctx context.Context) ([]*types.Message, error) {
	...
	for {
		...
		if edaxml.IsCRMsg(xmlBody) {
			...
			// CM_REV_IMP (AUFHEBUNG_CCMS_IMP / AUFHEBUNG_CCMI): Netzbetreiber revokes on
			// its own initiative (impossibility, or a Zählpunkt-Lieferantenwechsel
			// resetting third-party consents).
			process := "CM_REV_CUS"
			edaMsg = &types.Message{
				Process:    process,
				Direction:  types.DirectionInbound,
			...
			if r, err := edaxml.ParseCMRevoke(xmlBody); err == nil {
				edaMsg.From = r.From
				edaMsg.To = r.To
				if r.MessageCode == "AUFHEBUNG_CCMS_IMP" || r.MessageCode == "AUFHEBUNG_CCMI" {
