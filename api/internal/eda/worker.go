...
// reprocessPendingCRMsgs picks up DATEN_CRMSG messages stored in eda_messages with
// status 'pending' and processes them. Used on startup to handle messages that were
// stored but not yet processed (e.g. after a restart or a manual status reset).
func (w *Worker) reprocessPendingCRMsgs(ctx context.Context) {
	pending, err := w.edaMsgRepo.FetchPendingCRMsgs(ctx)
	if err != nil {
		w.log.Error("reprocess: failed to fetch pending CR_MSGs", "error", err)
		return
	}
	if len(pending) == 0 {
		return
	}
	w.log.Info("reprocessing pending DATEN_CRMSG messages", "count", len(pending))
	for _, pm := range pending {
		...
		if err != nil {
			...
			if markErr := w.edaMsgRepo.MarkError(ctx, pm.ID, err.Error()); markErr != nil {
				w.log.Warn("reprocess: failed to mark error", "id", pm.ID, "error", markErr)
			}
			continue
		}
		if eegID != uuid.Nil {
			if updateErr := w.edaMsgRepo.UpdateEegID(ctx, pm.ID, eegID); updateErr != nil {
				w.log.Warn("reprocess: failed to set eeg_id", "id", pm.ID, "error", updateErr)
			}
		}
		if markErr := w.edaMsgRepo.MarkProcessed(ctx, pm.ID); markErr != nil {
			w.log.Warn("reprocess: failed to mark processed", "id", pm.ID, "error", markErr)
		}
	}
	w.log.Info("reprocessing pending DATEN_CRMSG messages done", "count", len(pending))
}
...
// meteringPointTagRe matches <MeteringPoint> under any XML namespace prefix
// (cp:, ct:, ns0:, ns1:, or none) — the tag name is consistent across every EDA
// document type (CMRequest, CPRequest, CMRevoke, ConsumptionRecord, ECMPList).
var meteringPointTagRe = regexp.MustCompile(`<[A-Za-z0-9]*:?MeteringPoint[^>]*>([^<]+)</[A-Za-z0-9]*:?MeteringPoint>`)

// extractZaehlpunkte pulls all distinct Zählpunkt/ECID values out of a raw EDA XML
// payload for storage in eda_messages.zaehlpunkt. Most message types carry exactly one;
// ECMPList (Zählpunktliste) responses can carry many, so all distinct values are
// space-joined. Storing this separately lets the messages list be filtered by Zählpunkt
// via an indexed lookup instead of an ILIKE scan over the (often >100KB) xml_payload.
func extractZaehlpunkte(xmlPayload string) string {
	matches := meteringPointTagRe.FindAllStringSubmatch(xmlPayload, -1)
	if len(matches) == 0 {
		return ""
	}
...
func (w *Worker) storeInboundMessage(ctx context.Context, msg *types.Message) (uuid.UUID, error) {
	q := `INSERT INTO eda_messages
	        (message_id, process, message_type, subject, body, direction, status, xml_payload, error_msg, from_address, to_address, zaehlpunkt)
	...
	err := w.db.QueryRow(ctx, q,
		...
		msg.From,
		msg.To,
		extractZaehlpunkte(msg.XMLPayload),
	).Scan(&id)
	if err != nil {
...
func (w *Worker) processCRMsg(ctx context.Context, msg *types.Message) (uuid.UUID, error) {
	...
	// Look up the currently active meter point for this Zählpunkt — used only for the
	...
	// Plausibility check: the OBIS codes in the message themselves imply a direction
	// (consumption vs. generation schema) independent of what we have stored for this
	...
	// whole message: a Zählpunkt can be recycled across members (Mieterwechsel), and a
	// CR_MSG can carry late/corrected data that predates such a handover. Resolving once
	// via "whichever row is currently active" would silently attach the old member's
	// readings to the new member's row (or vice versa).
	mpCache := map[time.Time]uuid.UUID{}
	mpNotFound := map[time.Time]bool{}
	resolve := func(ts time.Time) (uuid.UUID, bool) {
		if id, ok := mpCache[ts]; ok {
			return id, true
		}
		if mpNotFound[ts] {
			return uuid.Nil, false
		}
		resolved, err := w.meterPointRepo.GetByZaehlpunktAtTime(ctx, knownEegID, record.Zaehlpunkt, ts)
		if err != nil {
			w.log.Warn("CR_MSG: no meter point covers this Zählpunkt at the reading's timestamp — skipping reading",
				"zaehlpunkt", record.Zaehlpunkt,
				"ts", ts,
				"error", err,
			)
...
// mapping in buildReadingsFromCRMsg); P.01T (or old-schema 2.9.0 P.01) is generation-only.
// Returns "" when the record carries no direction signal (e.g. only G.01/G.01T totals) or a
// contradictory one (both signals present) — callers should treat "" as "no opinion".
func detectSchemaDirection(record *edaxml.CRMsgRecord) string {
	sawConsumption := false
	sawGeneration := false
	for _, block := range record.Energies {
		for _, ed := range block.Data {
			mc := ed.MeterCode
			switch {
			case strings.Contains(mc, "G.02"), strings.Contains(mc, "G.03"):
				sawConsumption = true
			case strings.Contains(mc, "P.01T"):
				sawGeneration = true
			case strings.Contains(mc, "P.01") && strings.Contains(mc, "2.9.0"):
				sawGeneration = true
			}
		}
	}
	switch {
	case sawConsumption && !sawGeneration:
		return "CONSUMPTION"
	case sawGeneration && !sawConsumption:
		return "GENERATION"
	default:
		return ""
	}
...
func (w *Worker) processCMNotification(ctx context.Context, msgID uuid.UUID, msg *types.Message) (uuid.UUID, error) {
	...
	if result.CustomerName2 != "" {
		meterOwnerName = result.CustomerName2 + " " + result.CustomerName1
	}
	if err := w.edaProcRepo.UpdateResponseInfo(ctx, proc.ID, result.ResponseCodes, meterOwnerName, result.PortalApprovalURL); err != nil {
		w.log.Warn("CMNotification: failed to save response info", "process_id", proc.ID, "error", err)
	}
	...
	case "ANTWORT_ECON":
		...
		// Code 182 = kein fernauslbarer Zähler. Per §16e ElWOG muss der NB innerhalb
		// von 2 Monaten einen Smart Meter einbauen. Mitglied per E-Mail informieren.
	...
	case "AUFHEBUNG_CCMS_ABGEL":
		...
		newStatus = "rejected"
		errMsg = fmt.Sprintf("AUFHEBUNG_CCMS_ABGEL response_codes=%v", result.ResponseCodes)
	default:
	...
	if newStatus == "completed" && proc.ProcessType == "CM_REV_SP" &&
		...
		proc.ValidFrom != nil {
		if err := w.meterPointRepo.UpdateAbgemeldetAm(ctx, *proc.MeterPointID, proc.EegID, proc.Zaehlpunkt, *proc.ValidFrom, "Widerruf bestätigt (CM_REV_SP)", "eda"); err != nil {
...
//   - CM_REV_IMP (AUFHEBUNG_CCMS_IMP / AUFHEBUNG_CCMI): Netzbetreiber revokes on its own
//     initiative — e.g. impossibility, or a Zählpunkt-Lieferantenwechsel resetting
...
// notification for the involuntary variants. If no matching process exists (the
// Netzbetreiber revoked unsolicited, without us having sent our own CM_REV_SP), falls
...
func (w *Worker) processCMRevoke(ctx context.Context, msg *types.Message) (uuid.UUID, error) {
	...
	isIMP := result.MessageCode == "AUFHEBUNG_CCMS_IMP" || result.MessageCode == "AUFHEBUNG_CCMI"
	...
	proc, err := w.edaProcRepo.GetByConversationID(ctx, convID)
	if err != nil {
		// No matching outbound process — this is an unsolicited revoke initiated by the
	...
	if result.ConsentEnd != "" && proc.MeterPointID != nil && *proc.MeterPointID != uuid.Nil {
		if consentEndDate, parseErr := time.Parse("2006-01-02", result.ConsentEnd); parseErr == nil {
			if err := w.meterPointRepo.UpdateAbgemeldetAm(ctx, *proc.MeterPointID, proc.EegID, proc.Zaehlpunkt, consentEndDate.UTC(), errMsg, "eda"); err != nil {
	...
	// For the involuntary variants (NB-initiated): notify the operator.
	if isIMP {
...
// processCMRevokeUnsolicited handles a CMRevoke for which no matching outbound
// eda_process exists — the Netzbetreiber revoked consent on its own initiative
// (e.g. a Zählpunkt-Lieferantenwechsel resets EEG participation consent) rather than
// confirming a CM_REV_SP we sent. Matches the meter point directly by Zählpunkt,
// verified against the stored ConsentId to avoid acting on a stale/unrelated message.
func (w *Worker) processCMRevokeUnsolicited(ctx context.Context, result *edaxml.CMRevokeDoc, isIMP bool) (uuid.UUID, error) {
	mp, err := w.meterPointRepo.GetByZaehlpunkt(ctx, result.MeteringPoint)
	if err != nil || mp.ConsentID == "" || mp.ConsentID != result.ConsentID {
		w.log.Warn("CMRevoke: no matching EDA process or meter point found",
			"message_code", result.MessageCode,
	...
	if result.ConsentEnd == "" {
		w.log.Warn("CMRevoke: unsolicited revoke has no ConsentEnd — cannot close registration period",
			"meter_point_id", mp.ID, "metering_point", result.MeteringPoint)
		return mp.EegID, nil
	}
	consentEndDate, parseErr := time.Parse("2006-01-02", result.ConsentEnd)
	if parseErr != nil {
		w.log.Warn("CMRevoke: invalid ConsentEnd date",
			"consent_end", result.ConsentEnd, "error", parseErr)
		return mp.EegID, nil
	}

	if err := w.meterPointRepo.UpdateAbgemeldetAm(ctx, mp.ID, mp.EegID, mp.Zaehlpunkt, consentEndDate.UTC(), reason, "eda"); err != nil {
		w.log.Warn("CMRevoke: failed to set abgemeldet_am (unsolicited)",
			"meter_point_id", mp.ID, "consent_end", result.ConsentEnd, "error", err)
	} else {
		w.log.Info("CMRevoke: abgemeldet_am set on meter point (unsolicited Netzbetreiber revoke)",
			"meter_point_id", mp.ID, "message_code", result.MessageCode,
			"consent_end", result.ConsentEnd, "reason", reason)
	}
...
<p style="color: #94a3b8; font-size: 12px;">Diese Nachricht wurde automatisch generiert.</p>
	...
	smtpCfgEDA := invoicepkg.SMTPConfig{Host: eeg.SMTPHost, From: eeg.SMTPFrom, Username: eeg.SMTPUser, Password: eeg.SMTPPassword}
	if err := invoicepkg.SendLogged(ctx, w.emailLogRepo, smtpCfgEDA, eeg.ID, "eda_error", eeg.SMTPFrom, subject, nil, nil, []byte(msgBuilder.String())); err != nil {
...
// notifyDirectionMismatch alerts the EEG operator when a Zählpunkt's incoming CR_MSG OBIS
// schema doesn't match its stored Energierichtung — a strong signal that the meter point was
// registered with the wrong direction (e.g. a prosumer's two Zählpunkte swapped during
// onboarding, or during a manual EDA re-registration; see detectSchemaDirection). Idempotent
// per meter point via direction_mismatch_notified_at.
func (w *Worker) notifyDirectionMismatch(ctx context.Context, mp *domain.MeterPoint, detectedDirection string) {
	// Re-check idempotency close to send time — another goroutine may already be sending.
	notifiedAt, err := w.meterPointRepo.GetDirectionMismatchNotifiedAt(ctx, mp.ID)
	if err != nil {
		w.log.Warn("direction mismatch notify: lookup failed", "meter_point_id", mp.ID, "error", err)
		return
	}
	if notifiedAt != nil {
		return
	}
	eeg, err := w.eegRepo.GetByIDInternal(ctx, mp.EegID)
	if err != nil || eeg.SMTPHost == "" || eeg.SMTPFrom == "" || eeg.IsDemo {
		return
	}
	memberName := ""
	...
	subject := fmt.Sprintf("[EDA Hinweis] Zählpunkt %s: Energierichtung stimmt evtl. nicht", mp.Zaehlpunkt)
	htmlBody := fmt.Sprintf(`<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1e293b;">
<h2 style="color: #b45309;">Mögliche Zählpunkt-Verwechslung</h2>
<p>Für den Zählpunkt <strong><code>%s</code></strong> (Energiegemeinschaft <strong>%s</strong>%s) sind vom Netzbetreiber Messdaten eingetroffen, deren Format nicht zur bei uns hinterlegten Energierichtung passt.</p>
<table style="border-collapse: collapse; width: 100%%; font-size: 14px;">
  <tr><td style="padding: 6px 12px; background: #f1f5f9; font-weight: 600; width: 40%%;">Bei uns hinterlegt</td>
      <td style="padding: 6px 12px;">%s</td></tr>
  <tr><td style="padding: 6px 12px; background: #f1f5f9; font-weight: 600;">Laut empfangenen Messdaten</td>
      <td style="padding: 6px 12px; color: #b45309; font-weight: 600;">%s</td></tr>
</table>
<p style="margin-top: 16px;">Das deutet darauf hin, dass dieser Zählpunkt mit der falschen Energierichtung angemeldet wurde — z.B. weil bei einem Mitglied mit zwei Zählpunkten (Verbrauch + Einspeisung) die beiden vertauscht wurden. Bitte prüfen und ggf. die Energierichtung in den Zählpunkt-Stammdaten korrigieren.</p>
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<p style="color: #94a3b8; font-size: 12px;">Diese Nachricht wurde automatisch generiert und wird pro Zählpunkt nur einmal versendet.</p>
	...
	if err := invoicepkg.SendLogged(ctx, w.emailLogRepo, smtpCfg, eeg.ID, "eda_direction_mismatch", eeg.SMTPFrom, subject, nil, nil, []byte(msgBuilder.String())); err != nil {
		w.log.Warn("direction mismatch notification email failed", "meter_point_id", mp.ID, "error", err)
		return
	}
	w.log.Info("direction mismatch notification sent",
	...
	if err := w.meterPointRepo.SetDirectionMismatchNotified(ctx, mp.ID); err != nil {
		w.log.Warn("failed to set direction_mismatch_notified_at", "meter_point_id", mp.ID, "error", err)
	}
}

// sendSmartmeterInfoEmail notifies the member that their meter is not yet remotely readable
// and that the Netzbetreiber is obligated to install a smart meter within 2 months (§16e ElWOG).
// Called in a goroutine when ANTWORT_ECON contains response code 182.
// Idempotent: skips if customer_notified_at is already set.
func (w *Worker) sendSmartmeterInfoEmail(ctx context.Context, proc *domain.EDAProcess, portalApprovalURL string) {
	if proc.MeterPointID == nil || *proc.MeterPointID == uuid.Nil {
		return
	}
	mp, err := w.meterPointRepo.GetByID(ctx, *proc.MeterPointID)
	if err != nil {
		w.log.Warn("smartmeter info email: meter point not found", "meter_point_id", *proc.MeterPointID, "error", err)
		return
	}
	member, err := w.memberRepo.GetByID(ctx, mp.MemberID)
	if err != nil || member.Email == "" {
		w.log.Warn("smartmeter info email: member not found or no email", "member_id", mp.MemberID, "error", err)
		return
	}
	eeg, err := w.eegRepo.GetByIDInternal(ctx, proc.EegID)
	...
	// Re-check idempotency: another goroutine may have already sent the notification.
	freshProc, err := w.edaProcRepo.GetByID(ctx, proc.ID)
	if err == nil && freshProc.CustomerNotifiedAt != nil {
		return
	}
	...
	portalSection := ""
	if portalApprovalURL != "" {
		portalSection = fmt.Sprintf(`<p>Falls Sie die Zustimmung zur Datenfreigabe noch nicht erteilt haben, können Sie dies über das Portal Ihres Netzbetreibers nachholen:</p>
...
<head><meta charset="utf-8"></head>
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1e293b;">
<h2 style="color: #1e40af;">Ihre Anmeldung wird bearbeitet</h2>
<p>Liebe(r) %s,</p>
<p>Ihre Anmeldung für den Zählpunkt <strong><code>%s</code></strong> bei der Energiegemeinschaft <strong>%s</strong> wurde vom Netzbetreiber entgegengenommen.</p>
<p>Ihr Zählpunkt verfügt derzeit noch über <strong>kein fernauslesbares intelligentes Messgerät</strong> (Smartmeter). Das ist kein Problem — gemäß <strong>§&nbsp;16e ElWOG</strong> ist Ihr Netzbetreiber verpflichtet, innerhalb von <strong>2&nbsp;Monaten</strong> ein intelligentes Messgerät einzubauen.</p>
<p>Sobald der Smartmeter installiert und Ihre Zustimmung zur Datenfreigabe vorliegt, wird Ihre Teilnahme an der Energiegemeinschaft automatisch aktiv. Sie erhalten dann eine weitere Bestätigungs-E-Mail.</p>
%s
<p style="color: #475569; font-size: 14px;"><strong>Sie müssen derzeit nichts weiter unternehmen.</strong></p>
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<p style="color: #94a3b8; font-size: 12px;">Bei Fragen wenden Sie sich bitte direkt an die Energiegemeinschaft <strong>%s</strong>.</p>
	...
	memID := member.ID
	smtpCfg := invoicepkg.SMTPConfig{Host: eeg.SMTPHost, From: eeg.SMTPFrom, Username: eeg.SMTPUser, Password: eeg.SMTPPassword}
	if err := invoicepkg.SendLogged(ctx, w.emailLogRepo, smtpCfg, eeg.ID, "eda_smartmeter_info", member.Email, subject, &memID, nil, []byte(msgBuilder.String())); err != nil {
		w.log.Warn("smartmeter info email failed", "member_id", member.ID, "error", err)
		return
	}
	w.log.Info("smartmeter info email sent (§16e ElWOG)", "member_id", member.ID, "zaehlpunkt", proc.Zaehlpunkt)

	if err := w.edaProcRepo.SetCustomerNotified(ctx, proc.ID); err != nil {
		w.log.Warn("failed to set customer_notified_at", "process_id", proc.ID, "error", err)
	}
}
...
<p style="color: #94a3b8; font-size: 12px;">Bei Fragen wenden Sie sich bitte direkt an die Energiegemeinschaft.</p>
	...
	memID := member.ID
	smtpCfgConf := invoicepkg.SMTPConfig{Host: eeg.SMTPHost, From: eeg.SMTPFrom, Username: eeg.SMTPUser, Password: eeg.SMTPPassword}
	if err := invoicepkg.SendLogged(ctx, w.emailLogRepo, smtpCfgConf, eeg.ID, "eda_confirmation", member.Email, subject, &memID, nil, []byte(msgBuilder.String())); err != nil {
