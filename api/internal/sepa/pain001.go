...
func GeneratePain001(eeg *domain.EEG, entries []Pain001Entry, execDate time.Time) ([]byte, error) {
	...
	// Filter: payment amount <= -0.005 (rounds to at least -0.01 €) and member has IBAN.
	// For RC credit notes the payment is the net amount (VAT remitted to Finanzamt by EEG).
	var txs []Pain001Entry
	for _, e := range entries {
		if sepaPayAmount(e.Invoice, e.Member) <= -0.005 && e.Member.IBAN != "" {
			txs = append(txs, e)
		}
	}
	if len(txs) == 0 {
	...
	for _, tx := range txs {
		amount := -sepaPayAmount(tx.Invoice, tx.Member)
...
func sepaPayAmount(inv *domain.Invoice, m *domain.Member) float64 {
	if isGenerationRC(m) {
		return inv.TotalAmount + inv.GenerationVatAmount
	}
...
func isGenerationRC(m *domain.Member) bool {
	...
	switch m.BusinessRole {
	case "privat", "gemeinde_hoheitlich", "landwirt_pauschaliert":
		return false
	}
	return m.UidNummer != ""
}
