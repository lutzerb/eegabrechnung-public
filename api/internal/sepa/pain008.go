...
func GeneratePain008(eeg *domain.EEG, entries []Pain008Entry) ([]byte, error) {
	...
	// Filter: amount >= 0.005 (rounds to at least 0.01 €) and member has IBAN
	var txs []Pain008Entry
	for _, e := range entries {
		if e.Invoice.TotalAmount >= 0.005 && e.Member.IBAN != "" {
			txs = append(txs, e)
		}
	}
	if len(txs) == 0 {
	...
	collectionDate := Pain008CollectionDate(eeg, txs).Format("2006-01-02")

	var ctrlSum float64
	for _, tx := range txs {
		ctrlSum += sepaPayAmount(tx.Invoice, tx.Member)
	}
	...
	for _, tx := range txs {
		...
		txElems = append(txElems, drctDbtTxInf{
			...
			InstdAmt:  instdAmt{Ccy: "EUR", Value: fmt.Sprintf("%.2f", sepaPayAmount(tx.Invoice, tx.Member))},
			DrctDbtTx: dt,
			DbtrAgt:   dag,
...
// Pain008CollectionDate returns the SEPA direct-debit collection date for the given entries:
// latest invoice creation date + SepaPreNotificationDays, clamped to at least tomorrow.
func Pain008CollectionDate(eeg *domain.EEG, entries []Pain008Entry) time.Time {
...
// NextWorkday returns the next weekday (Mon–Fri) strictly after t.
