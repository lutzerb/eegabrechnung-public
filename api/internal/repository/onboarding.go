...
func (r *OnboardingRepository) GetByToken(ctx context.Context, token string) (*domain.OnboardingRequest, error) {
	q := `SELECT id, eeg_id, status, name1, name2, email, phone, strasse, plz, ort,
	             ...
	             meter_points, beitritts_datum, contract_accepted_at, contract_ip,
	             magic_token, magic_token_expires_at, admin_notes, referral_source, referral_source_note,
	             converted_member_id, referred_by_member_id,
...
// FindPendingByEmailAndEEG finds the most recent open request by email for a given EEG.
// Covers all non-terminal statuses: pending, approved, admin_created.
...
// Confirm transitions an admin_created request to pending and records the member's acceptance.
func (r *OnboardingRepository) Confirm(ctx context.Context, id uuid.UUID, ip string) error {
	_, err := r.db.Exec(ctx, `
		UPDATE onboarding_requests
		SET status = 'pending', contract_accepted_at = now(), contract_ip = $1, updated_at = now()
		WHERE id = $2`,
		ip, id,
	)
	if err != nil {
...
func scanOnboarding(row interface {
	...
	err := row.Scan(
		...
		&req.ConvertedMemberID, &req.ReferredByMemberID,
		&req.ReminderSentAt,
...
// AbandonedEmailVerification holds data for a person who verified their email
// but never completed the onboarding form.
type AbandonedEmailVerification struct {
	ID    uuid.UUID
	EegID uuid.UUID
	Email string
	Name1 string
...
// FindAbandonedEmailVerifications returns email verifications older than delay
// where no onboarding_request was ever submitted for that email+eeg, and no
// reminder has been sent yet. Skips demo EEGs.
func (r *OnboardingRepository) FindAbandonedEmailVerifications(ctx context.Context, delay time.Duration) ([]AbandonedEmailVerification, error) {
	q := `
		SELECT DISTINCT ON (ev.eeg_id, ev.email)
		       ev.id, ev.eeg_id, ev.email, ev.name1
		FROM onboarding_email_verifications ev
		JOIN eegs e ON e.id = ev.eeg_id
		WHERE ev.created_at < now() - $1::interval
		  AND ev.reminder_sent_at IS NULL
		  AND e.is_demo = false
		  AND NOT EXISTS (
		    SELECT 1 FROM onboarding_requests o
		    WHERE o.eeg_id = ev.eeg_id
		      AND lower(o.email) = lower(ev.email)
		  )
		ORDER BY ev.eeg_id, ev.email, ev.created_at DESC`
	...
	defer rows.Close()
	var result []AbandonedEmailVerification
	for rows.Next() {
		var a AbandonedEmailVerification
		if err := rows.Scan(&a.ID, &a.EegID, &a.Email, &a.Name1); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
