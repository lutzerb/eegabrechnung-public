...
type MeterPointRepository struct {
	db      *pgxpool.Pool
	periods *MeterPointRegistrationPeriodRepository
}
...
func NewMeterPointRepository(db *pgxpool.Pool) *MeterPointRepository {
	return &MeterPointRepository{db: db, periods: NewMeterPointRegistrationPeriodRepository(db)}
}

// ErrHandoverMissingRegistriertSeit is returned by Upsert when a Stammdaten row implies a
// Zählpunkt handover (Mitgliedswechsel) but carries no "Registriert seit" date to use as
// the old row's deregistration cutoff. Callers should surface this as a 400 to the admin
// rather than guessing a cutoff date.
var ErrHandoverMissingRegistriertSeit = errors.New("mitgliedswechsel erkannt, aber 'registriert seit' fehlt")

// HandoverInfo describes a Zählpunkt handover detected during a Stammdaten import: the
// Zählpunkt now belongs to a different member than the currently active row.
type HandoverInfo struct {
	Zaehlpunkt  string
	OldMemberID uuid.UUID
	NewMemberID uuid.UUID
	CutoffDate  time.Time
}

// Upsert inserts or updates a meter point from a Stammdaten import row.
//
//   - No active row yet for (eeg_id, zaehlpunkt) → plain insert (via Create).
//   - Active row belongs to the SAME member → update mutable attributes in place.
//     registriert_seit is deliberately NOT overwritten here (see UpdateRegistriertSeit
//     for the one correct way to change it) — a blank/stale date in a re-uploaded
//     Stammdaten sheet must not silently rewrite the activation date/registration
//     history for a member whose Zählpunkt didn't actually change hands.
//   - Active row belongs to a DIFFERENT member → Mitgliedswechsel / Zählpunkt handover
//     (see migration 070): the old row is closed (abgemeldet_am = the new row's
//     registriert_seit) and a brand-new row is created for the new member, instead of
//     blindly overwriting member_id on the same row — which would silently reattach all
//     of the old row's historical energy_readings to the new member and make the old
//     member's meter_points row (and history) disappear. Returns a non-nil *HandoverInfo
//     when a handover was performed, so the caller can surface it to the admin.
func (r *MeterPointRepository) Upsert(ctx context.Context, mp *domain.MeterPoint) (*HandoverInfo, error) {
	var existingID, existingMemberID uuid.UUID
	err := r.db.QueryRow(ctx,
		`SELECT id, member_id FROM meter_points WHERE eeg_id=$1 AND zaehlpunkt=$2 AND abgemeldet_am IS NULL`,
		mp.EegID, mp.Zaehlpunkt,
	).Scan(&existingID, &existingMemberID)

	switch {
	case errors.Is(err, pgx.ErrNoRows):
		return nil, r.Create(ctx, mp)

	case err != nil:
		return nil, fmt.Errorf("lookup existing meter point: %w", err)

	case existingMemberID == mp.MemberID:
		q := `UPDATE meter_points
		      SET energierichtung = $1, verteilungsmodell = $2, zugeteilte_menge_pct = $3, status = $4
		      WHERE id = $5
		      RETURNING id, created_at`
		if err := r.db.QueryRow(ctx, q,
			mp.Energierichtung, mp.Verteilungsmodell, mp.ZugeteilteMenugePct, mp.Status, existingID,
		).Scan(&mp.ID, &mp.CreatedAt); err != nil {
			return nil, fmt.Errorf("update meter point: %w", err)
		}
		if mp.RegistriertSeit != nil {
			if err := r.UpdateRegistriertSeit(ctx, mp.ID, mp.EegID, mp.Zaehlpunkt, *mp.RegistriertSeit, mp.ConsentID); err != nil {
				return nil, fmt.Errorf("update registriert_seit: %w", err)
			}
		}
		return nil, nil
	...
	default:
		if mp.RegistriertSeit == nil {
			return nil, fmt.Errorf("zählpunkt %s: %w — bitte im Stammdaten-Sheet angeben", mp.Zaehlpunkt, ErrHandoverMissingRegistriertSeit)
		}
		cutoff := *mp.RegistriertSeit

		tx, err := r.db.Begin(ctx)
		if err != nil {
			return nil, fmt.Errorf("begin handover tx: %w", err)
		}
		defer tx.Rollback(ctx)

		if _, err := tx.Exec(ctx,
			`UPDATE meter_points SET abgemeldet_am = $1 WHERE id = $2`,
			cutoff.UTC().Truncate(24*time.Hour), existingID,
		); err != nil {
			return nil, fmt.Errorf("close old meter point: %w", err)
		}
		// Mirrors MeterPointRegistrationPeriodRepository.ClosePeriod, run against the same
		// tx (that repo's methods use the pool directly, not a tx) so the meter_points
		// close and the period close commit atomically together.
		if _, err := tx.Exec(ctx,
			...
			mp.EegID, mp.Zaehlpunkt, cutoff.UTC().Truncate(24*time.Hour), "stammdaten_handover", "import",
		); err != nil {
			return nil, fmt.Errorf("close registration period: %w", err)
		}

		insertQ := `INSERT INTO meter_points (member_id, eeg_id, zaehlpunkt, energierichtung, verteilungsmodell, zugeteilte_menge_pct, status, registriert_seit, generation_type)
		...
		if err := tx.QueryRow(ctx, insertQ,
			...
			return nil, fmt.Errorf("insert new meter point: %w", err)
		}
		// Mirrors MeterPointRegistrationPeriodRepository.OpenPeriod, run against the same tx.
		if _, err := tx.Exec(ctx,
			...
			mp.EegID, mp.Zaehlpunkt, mp.ID, cutoff.UTC().Truncate(24*time.Hour), mp.ConsentID, "import",
		); err != nil {
			return nil, fmt.Errorf("open registration period: %w", err)
		}

		if err := tx.Commit(ctx); err != nil {
			return nil, fmt.Errorf("commit handover tx: %w", err)
		}

		return &HandoverInfo{
			Zaehlpunkt:  mp.Zaehlpunkt,
			OldMemberID: existingMemberID,
			NewMemberID: mp.MemberID,
			CutoffDate:  cutoff,
		}, nil
	}
}
...
// GetDirectionMismatchNotifiedAt returns whether an OBIS-schema/Energierichtung mismatch
// alert has already been sent for this meter point (nil = not yet notified).
func (r *MeterPointRepository) GetDirectionMismatchNotifiedAt(ctx context.Context, meterPointID uuid.UUID) (*time.Time, error) {
	var t *time.Time
	err := r.db.QueryRow(ctx,
		`SELECT direction_mismatch_notified_at FROM meter_points WHERE id = $1`,
		meterPointID,
	).Scan(&t)
	return t, err
}

// SetDirectionMismatchNotified marks a meter point as having had a direction-mismatch alert sent.
func (r *MeterPointRepository) SetDirectionMismatchNotified(ctx context.Context, meterPointID uuid.UUID) error {
	_, err := r.db.Exec(ctx,
		`UPDATE meter_points SET direction_mismatch_notified_at = NOW() WHERE id = $1`,
		meterPointID,
	)
	return err
}
...
// UpdateRegistriertSeit sets the NB-confirmed activation date on a meter point and
// opens the corresponding registration period (see MeterPointRegistrationPeriodRepository).
// Also clears abgemeldet_am: a freshly confirmed EC_REQ_ONL supersedes any prior
// deregistration on this row (e.g. re-registering after a Zählpunkt-Lieferantenwechsel) —
// otherwise the meter point would stay permanently excluded from billing eligibility
// and gap alerts, which key off abgemeldet_am IS NULL.
func (r *MeterPointRepository) UpdateRegistriertSeit(ctx context.Context, meterPointID, eegID uuid.UUID, zaehlpunkt string, date time.Time, consentID string) error {
	_, err := r.db.Exec(ctx,
		`UPDATE meter_points SET registriert_seit = $1, abgemeldet_am = NULL WHERE id = $2`,
		date.UTC().Truncate(24*time.Hour), meterPointID,
	)
	if err != nil {
		return err
	}
	if err := r.periods.OpenPeriod(ctx, eegID, zaehlpunkt, meterPointID, date, consentID, "eda"); err != nil {
...
// UpdateAbgemeldetAm sets the NB-confirmed deregistration date on a meter point and
// closes the currently open registration period (see MeterPointRegistrationPeriodRepository).
func (r *MeterPointRepository) UpdateAbgemeldetAm(ctx context.Context, meterPointID, eegID uuid.UUID, zaehlpunkt string, date time.Time, reason, source string) error {
	_, err := r.db.Exec(ctx,
		`UPDATE meter_points SET abgemeldet_am = $1 WHERE id = $2`,
		date.UTC().Truncate(24*time.Hour), meterPointID,
	)
	...
	if err := r.periods.ClosePeriod(ctx, eegID, zaehlpunkt, date, reason, source); err != nil {
		return fmt.Errorf("close registration period: %w", err)
	}
	return nil
}

// ClosePeriodManual closes the open registration period for a Zählpunkt from a manual
// admin edit (not an EDA confirmation).
func (r *MeterPointRepository) ClosePeriodManual(ctx context.Context, eegID uuid.UUID, zaehlpunkt string, date time.Time, reason string) error {
	return r.periods.ClosePeriod(ctx, eegID, zaehlpunkt, date, reason, "manual")
}

// ReopenPeriodManual reopens the latest registration period for a Zählpunkt from a
// manual admin edit (clearing abgemeldet_am).
func (r *MeterPointRepository) ReopenPeriodManual(ctx context.Context, eegID uuid.UUID, zaehlpunkt string) error {
...
// ExistsByZaehlpunktInEEG returns true if an *active* (abgemeldet_am IS NULL) meter point
// with that Zählpunkt ID already exists in the EEG. Historical (deregistered) rows are
// ignored so that a Zählpunkt can be reused by a new member after the previous one left.
func (r *MeterPointRepository) ExistsByZaehlpunktInEEG(ctx context.Context, eegID uuid.UUID, zaehlpunkt string) (bool, error) {
	var exists bool
	err := r.db.QueryRow(ctx,
		`SELECT EXISTS(SELECT 1 FROM meter_points WHERE eeg_id=$1 AND zaehlpunkt=$2 AND abgemeldet_am IS NULL)`,
		eegID, zaehlpunkt,
	).Scan(&exists)
	return exists, err
}

// GetByZaehlpunkt returns the active (abgemeldet_am IS NULL) meter point for the given
// Zählpunkt string. If a Zählpunkt has been recycled across members, older deregistered
// rows are ignored — only the currently active one is returned.
func (r *MeterPointRepository) GetByZaehlpunkt(ctx context.Context, zaehlpunkt string) (*domain.MeterPoint, error) {
	q := `SELECT ` + selectMeterPoint + ` FROM meter_points WHERE zaehlpunkt = $1 AND abgemeldet_am IS NULL`
...
// GetByZaehlpunktAtTime returns the meter_points row that was active for the given
// Zählpunkt at the given timestamp — i.e. the row whose registriert_seit/abgemeldet_am
// window contains ts — not "whichever row is active right now". This is the correct
// resolver for attaching a historical energy reading to a Zählpunkt string, since a
// Zählpunkt can be recycled across members and readings can arrive out of order
// (late/backfilled EDA messages, XLSX imports spanning a handover). Returns an error
// (no rows) if no row's window covers ts — callers should skip that single reading
// rather than guess, since a "nearest row" fallback would reintroduce misattribution.
//
// registriert_seit/abgemeldet_am are Vienna-local calendar dates (same convention as
// MissingIntervals/MissingIntervalDetails), so the DATE→timestamptz conversion must use
...
func (r *MeterPointRepository) GetByZaehlpunktAtTime(ctx context.Context, eegID uuid.UUID, zaehlpunkt string, ts time.Time) (*domain.MeterPoint, error) {
	q := `SELECT ` + selectMeterPoint + `
	      ...
	      LIMIT 1`
	var mp domain.MeterPoint
	if err := scanMeterPoint(r.db.QueryRow(ctx, q, eegID, zaehlpunkt, ts), &mp); err != nil {
		return nil, fmt.Errorf("scan: %w", err)
	}
	return &mp, nil
}

// GetLatestByZaehlpunktInEEG returns the most recently created meter point row for a
// Zählpunkt within an EEG, regardless of abgemeldet_am. Used to link an outbound
// EC_REQ_ONL to the right row when re-registering a Zählpunkt that is currently
// deregistered (e.g. after a Zählpunkt-Lieferantenwechsel) — GetByZaehlpunkt would
// find nothing in that case since it only matches active rows.
func (r *MeterPointRepository) GetLatestByZaehlpunktInEEG(ctx context.Context, eegID uuid.UUID, zaehlpunkt string) (*domain.MeterPoint, error) {
	q := `SELECT ` + selectMeterPoint + ` FROM meter_points WHERE eeg_id = $1 AND zaehlpunkt = $2 ORDER BY created_at DESC LIMIT 1`
	var mp domain.MeterPoint
	if err := scanMeterPoint(r.db.QueryRow(ctx, q, eegID, zaehlpunkt), &mp); err != nil {
		return nil, fmt.Errorf("scan: %w", err)
	}
	return &mp, nil
}
...
func (r *MeterPointRepository) Create(ctx context.Context, mp *domain.MeterPoint) error {
	...
	if mp.RegistriertSeit != nil {
		if err := r.periods.OpenPeriod(ctx, mp.EegID, mp.Zaehlpunkt, mp.ID, *mp.RegistriertSeit, mp.ConsentID, "eda"); err != nil {
			return fmt.Errorf("open registration period: %w", err)
		}
	}
	return nil
...
func (r *MeterPointRepository) Update(ctx context.Context, mp *domain.MeterPoint) error {
	q := `UPDATE meter_points SET
	        energierichtung=$1, verteilungsmodell=$2, zugeteilte_menge_pct=$3, status=$4, generation_type=$5, notes=$6, abgemeldet_am=$7
	      WHERE id=$8`
	_, err := r.db.Exec(ctx, q,
		mp.Energierichtung, mp.Verteilungsmodell, mp.ZugeteilteMenugePct, mp.Status, mp.GenerationType, mp.Notes, mp.AbgemeldetAm, mp.ID,
	)
	if err != nil {
...
// CountActiveByMember returns the number of active (not deregistered) meter points for a member.
// Used to compute the per-Zählpunkt fee during billing.
func (r *MeterPointRepository) CountActiveByMember(ctx context.Context, memberID uuid.UUID) (int, error) {
	var count int
	err := r.db.QueryRow(ctx,
		`SELECT COUNT(*) FROM meter_points WHERE member_id = $1 AND abgemeldet_am IS NULL`,
		memberID).Scan(&count)
	return count, err
}
