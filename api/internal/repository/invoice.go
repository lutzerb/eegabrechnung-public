...
const invoiceCols = `id, member_id, eeg_id, period_start, period_end, total_kwh, total_amount,
	...
	sepa_return_at, sepa_return_reason, sepa_return_note, paid_at`
...
func scanInvoice(row interface{ Scan(...any) error }, inv *domain.Invoice) error {
	...
	err := row.Scan(
		...
		&inv.SepaReturnAt, &sepaReturnReason, &sepaReturnNote, &inv.PaidAt,
	)
...
// SetPaid marks an invoice as paid and records the bank booking date.
func (r *InvoiceRepository) SetPaid(ctx context.Context, id uuid.UUID, paidAt time.Time) error {
	q := `UPDATE invoices SET status='paid', paid_at=$2 WHERE id=$1 AND status NOT IN ('paid','cancelled')`
	_, err := r.db.Exec(ctx, q, id, paidAt)
	return err
}
