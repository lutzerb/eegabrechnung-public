...
func (r *EARepository) DeleteUVA(ctx context.Context, id, eegID uuid.UUID) error {
	_, err := r.db.Exec(ctx,
		`DELETE FROM ea_uva_perioden WHERE id=$1 AND eeg_id=$2`,
		id, eegID,
	)
	return err
}
...
func (r *EARepository) CalcUVAKennzahlen(ctx context.Context, eegID uuid.UUID, von, bis time.Time) (*domain.EAUVAPeriode, error) {
	...
	q := `SELECT ust_code, richtung, SUM(ABS(betrag_netto)), SUM(ABS(ust_betrag))
	      FROM ea_buchungen
	      WHERE eeg_id=$1
	        AND COALESCE(zahlung_datum, beleg_datum) >= $2
	        AND COALESCE(zahlung_datum, beleg_datum) <= $3
	        AND (zahlung_datum IS NOT NULL OR beleg_datum IS NOT NULL)
...
// InsertBankTransaktion inserts a bank transaction. Returns (true, nil) when inserted,
// (false, nil) when skipped by either dedup index (with-ref or no-ref variant).
func (r *EARepository) InsertBankTransaktion(ctx context.Context, t *domain.EABankTransaktion) (bool, error) {
	...
	if err != nil {
		if err == pgx.ErrNoRows {
			return false, nil // duplicate skipped by dedup index
		}
		return false, err
	}
	return true, nil
...
func (r *EARepository) QueryInvoicesForImport(ctx context.Context, eegID uuid.UUID, von, bis *time.Time) ([]domain.EAImportPreviewRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&invoiceID, &invoiceNr, &datum, &docType,
			...
			&eegUseVat, &pdfPath, &paidAt); err != nil {
			return nil, fmt.Errorf("scan: %w", err)
		}
		...
		case docType == "credit_note" || (consumptionNet == 0 && generationNet > 0):
			...
			result = append(result, domain.EAImportPreviewRow{
				InvoiceID: invoiceID, InvoiceNr: invoiceNrStr, Datum: datum, PaidAt: paidAt,
