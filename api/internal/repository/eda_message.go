...
// PendingCRMsg is a stored DATEN_CRMSG message that has not yet been processed.
type PendingCRMsg struct {
	ID   uuid.UUID
	Body string
}

// FetchPendingCRMsgs returns all DATEN_CRMSG messages with status 'pending', ordered by creation time.
func (r *EDAMessageRepository) FetchPendingCRMsgs(ctx context.Context) ([]PendingCRMsg, error) {
	rows, err := r.db.Query(ctx,
		`SELECT id, xml_payload FROM eda_messages
		 WHERE message_type = 'DATEN_CRMSG' AND status = 'pending'
		 ORDER BY created_at`,
	)
	if err != nil {
		return nil, fmt.Errorf("fetch pending CR_MSGs: %w", err)
	}
	defer rows.Close()
	var result []PendingCRMsg
	for rows.Next() {
		var m PendingCRMsg
...
// CountByEEG returns the total number of EDA messages for one EEG, optionally filtered.
// direction: "inbound"|"outbound"|"" (empty = all)
// processType: message_type / process code, "" = all
// zaehlpunkt: substring match against the zaehlpunkt column (extracted from <MeteringPoint>
// at ingest time — see extractZaehlpunkte in worker.go), "" = no filter
func (r *EDAMessageRepository) CountByEEG(ctx context.Context, eegID uuid.UUID, direction, processType, zaehlpunkt string) (int, error) {
	args := []any{eegID}
	where := "WHERE eeg_id = $1"
	if direction != "" {
	...
	if processType != "" {
		...
		where += fmt.Sprintf(" AND (process = $%d OR message_type = $%d)", len(args), len(args))
	}
	if zaehlpunkt != "" {
		args = append(args, "%"+zaehlpunkt+"%")
		where += fmt.Sprintf(" AND zaehlpunkt ILIKE $%d", len(args))
	}
	var count int
	err := r.db.QueryRow(ctx, `SELECT COUNT(*) FROM eda_messages `+where, args...).Scan(&count)
	if err != nil {
...
// ListByEEG returns EDA messages for a specific EEG, newest first, optionally filtered.
// direction: "inbound"|"outbound"|"" (empty = all)
// processType: message_type / process code, "" = all
// zaehlpunkt: substring match against the zaehlpunkt column, "" = no filter
func (r *EDAMessageRepository) ListByEEG(ctx context.Context, eegID uuid.UUID, direction, processType, zaehlpunkt string, limit, offset int) ([]domain.EDAMessage, error) {
	args := []any{eegID}
	where := "WHERE eeg_id = $1"
	if direction != "" {
		args = append(args, direction)
	...
	q := `SELECT ` + edaMsgCols + ` FROM eda_messages ` + where +
