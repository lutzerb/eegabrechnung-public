...
type EEG struct {
	...
	EdaDisModel        string     `json:"eda_dis_model"`                 // ECDisModel declared to the Netzbetreiber for this community: S (statisch) | D (dynamisch, default)
...
// preserved whenever the IBAN changes so past mandates stay retrievable
// (e.g. for audit / due-diligence purposes).
type SepaMandateHistoryEntry struct {
	ID         uuid.UUID  `json:"id"`
	MemberID   uuid.UUID  `json:"member_id"`
	EegID      uuid.UUID  `json:"eeg_id"`
	IBAN       string     `json:"iban"`
	SignedAt   *time.Time `json:"signed_at,omitempty"`
	SignedIP   string     `json:"signed_ip"`
...
// MeterPointRegistrationPeriod is one Anmeldung/Abmeldung period for a Zählpunkt.
// Keyed by (eeg_id, zaehlpunkt) rather than meter_point_id so the history stays
// complete across Zählpunkt-recycling (a new meter_points row for a new member reusing
// the same Zählpunkt) as well as re-registration of the same row.
type MeterPointRegistrationPeriod struct {
	ID              uuid.UUID  `json:"id"`
	EegID           uuid.UUID  `json:"eeg_id"`
	Zaehlpunkt      string     `json:"zaehlpunkt"`
	MeterPointID    *uuid.UUID `json:"meter_point_id,omitempty"`
	RegistriertSeit time.Time  `json:"registriert_seit"`
	AbgemeldetAm    *time.Time `json:"abgemeldet_am,omitempty"`
	ConsentID       string     `json:"consent_id"`
	Source          string     `json:"source"` // eda | manual | backfill
	Reason          string     `json:"reason"`
	CreatedAt       time.Time  `json:"created_at"`
}
...
type MemberStat struct {
	...
	TotalAmount         float64   `json:"total_amount"`
	InvoiceCount        int       `json:"invoice_count"`
	L3ReadingCount      int       `json:"l3_reading_count"` // number of L3 (faulty) readings in the period; 0 in billed mode
}
...
type EDAProcess struct {
	...
	// Response info from inbound CMNotification (populated by worker on ANTWORT/ZUSTIMMUNG_ECON).
	ResponseCodes      []string   `json:"response_codes"`
	MeterOwnerName     string     `json:"meter_owner_name"`
	PortalApprovalURL  string     `json:"portal_approval_url"`
	CustomerNotifiedAt *time.Time `json:"customer_notified_at,omitempty"`
	CreatedAt          time.Time  `json:"created_at"`
...
type EnergyRow struct {
	...
	WhCommunity float64
	WhSelf      float64
	Quality     string // L0/L1/L2/L3, from the "MM" quality column paired with the value column; "" if the file carries none
}
...
// A schedule with MemberID == nil is the EEG-wide default; one with MemberID set
// is a member-specific Individualtarif that overrides individual EEG fee fields when
// the corresponding *Override pointer is non-nil (nil = fall back to the EEG default).
type TariffSchedule struct {
	...
	// Member-specific overrides (only meaningful when MemberID != nil). nil = use EEG default.
...
// EmailLogEntry records one outgoing email attempt.
