...
func (h *EAHandler) DeleteUVA(w http.ResponseWriter, r *http.Request) {
	eegID, ok := h.parseEegID(r)
	if !ok {
		jsonError(w, "invalid EEG ID", http.StatusBadRequest)
		return
	...
	if err := h.eaRepo.DeleteUVA(r.Context(), uvaID, eegID); err != nil {
		jsonError(w, "delete failed", http.StatusInternalServerError)
		return
	}
	w.WriteHeader(http.StatusNoContent)
...
func (h *EAHandler) ImportBank(w http.ResponseWriter, r *http.Request) {
	...
	for i := range transakts {
		...
		inserted, err := h.eaRepo.InsertBankTransaktion(r.Context(), &transakts[i])
		if err != nil {
	...
	jsonOK(w, map[string]any{"imported": imported, "skipped": skipped, "total": len(transakts)})
}
