...
func (h *BillingHandler) RunBilling(w http.ResponseWriter, r *http.Request) {
	...
	start, err := time.ParseInLocation("2006-01-02", req.PeriodStart, viennaLoc)
	if err != nil {
		jsonError(w, "invalid period_start format (expected YYYY-MM-DD)", http.StatusBadRequest)
		return
	}
	end, err := time.ParseInLocation("2006-01-02", req.PeriodEnd, viennaLoc)
	if err != nil {
		jsonError(w, "invalid period_end format (expected YYYY-MM-DD)", http.StatusBadRequest)
		return
	}
	// Include the full last day in Vienna time
	...
	if err != nil {
		...
		if errors.As(err, &gapErr) {
			...
			_ = json.NewEncoder(w).Encode(map[string]any{
				"error":   "Unvollständige Daten: fehlende L1/L2-Intervalle",
				"details": gapErr.Details,
			})
			return
		}
