...
func (h *ReportHandler) GetRawMemberEnergy(w http.ResponseWriter, r *http.Request) {
	...
	var zaehlpunkt *string
	if s := r.URL.Query().Get("zaehlpunkt"); s != "" {
		zaehlpunkt = &s
	}

	stats, err := h.reportRepo.RawMemberEnergy(r.Context(), eeg.ID, from, to, memberID, zaehlpunkt)
	if err != nil {
		jsonError(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *ReportHandler) GetEnergySummary(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := h.reportRepo.EnergySummary(r.Context(), eeg.ID, from, to, granularity, memberID, zaehlpunkt)
	if err != nil {
		jsonError(w, "query failed: "+err.Error(), http.StatusInternalServerError)
		return
	}
