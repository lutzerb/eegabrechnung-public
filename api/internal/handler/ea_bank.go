...
func ParseCAMT053(data []byte, eegID uuid.UUID) ([]domain.EABankTransaktion, error) {
	...
	for _, stmt := range doc.BkToCstmrStmt.Stmt {
		...
		for _, entry := range stmt.Ntry {
			...
			// Normalize AcctSvcrRef: "NOTPROVIDED" and whitespace-only are not real references.
			// Without this, bank charges with the same date/amount/NOTPROVIDED would be
			// incorrectly deduplicated by the partial unique index.
			ref := strings.TrimSpace(entry.AcctSvcrRef)
			if ref == "NOTPROVIDED" {
				ref = ""
			}
			...
			// Combine descriptions and names from all sub-transactions (SEPA batch entries
			// contain one Ntry with multiple TxDtls — one per payment within the batch).
			var descs, names []string
			for _, td := range entry.NtryDtls.TxDtls {
				desc := strings.TrimSpace(td.RmtInf.Ustrd)
				if desc == "" {
					desc = strings.TrimSpace(td.RmtInf.Strd.CdtrRefInf.Ref)
				}
				if desc != "" {
					descs = append(descs, desc)
				}
				if td.RltdPties.Dbtr.Nm != "" {
					names = append(names, td.RltdPties.Dbtr.Nm)
				} else if td.RltdPties.Cdtr.Nm != "" {
					names = append(names, td.RltdPties.Cdtr.Nm)
				}
				// Use EndToEndId as referenz fallback only for single-transaction entries,
				// since batch entries share one Ntry-level AcctSvcrRef.
				if t.Referenz == "" && len(entry.NtryDtls.TxDtls) == 1 {
					t.Referenz = strings.TrimSpace(td.Refs.EndToEndId)
				}
			}
			t.Verwendungszweck = strings.Join(descs, " | ")
			t.AuftraggeberEmpfaenger = strings.Join(names, ", ")

			// Mark reversed/storno entries visibly so they stand out in the journal.
			if strings.EqualFold(strings.TrimSpace(entry.RvslInd), "true") {
				t.Verwendungszweck = "[STORNO] " + t.Verwendungszweck
			}
