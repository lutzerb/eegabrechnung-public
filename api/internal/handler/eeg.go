...
func (h *EEGHandler) CreateEEG(w http.ResponseWriter, r *http.Request) {
	...
	if req.GemeinschaftTyp != "BEG" && req.Netzbetreiber == "" {
		jsonError(w, "netzbetreiber is required", http.StatusBadRequest)
		return
	}
	if !validInvoicePaymentNoticeMode(req.InvoicePaymentNoticeMode) {
		jsonError(w, "invalid invoice_payment_notice_mode", http.StatusBadRequest)
		return
	}
	if !validEdaDisModel(req.EdaDisModel) {
...
func (h *EEGHandler) UpdateEEG(w http.ResponseWriter, r *http.Request) {
	...
	if req.GemeinschaftTyp != "" {
		existing.GemeinschaftTyp = req.GemeinschaftTyp
	}
	...
	if req.EdaDisModel != "" {
		existing.EdaDisModel = req.EdaDisModel
	}
...
func (h *EEGHandler) ExportStammdaten(w http.ResponseWriter, r *http.Request) {
	...
	// Build member lookup for joining with meter points.
	memberMap := make(map[uuid.UUID]*domain.Member, len(members))
	for i := range members {
	...
	// Sheet 1: Mitglieder — one row per member.
	sheet1 := "Mitglieder"
	...
	for row, m := range members {
		r := row + 2
		vals := []any{
			m.MitgliedsNr, m.Name1, m.Name2, m.Email, m.IBAN, m.BusinessRole,
			m.Strasse, m.Plz, m.Ort, m.UidNummer, m.Status,
		}
	...
	// Sheet 2: Zählpunkte — one row per meter point, Mitglieds-Nr as FK.
	sheet2 := "Zählpunkte"
	f.NewSheet(sheet2)
	headers2 := []string{
	...
	rowNum := 2
	for _, mp := range mps {
		if mp.Status == "INACTIVE" || mp.Status == "DEACTIVATED" {
			continue
		}
		m, ok := memberMap[mp.MemberID]
		if !ok {
			continue
		}
		since := ""
		...
		vals := []any{
			m.MitgliedsNr, mp.Zaehlpunkt, mp.Energierichtung, mp.Verteilungsmodell,
			mp.ZugeteilteMenugePct, mp.Status, since,
		}
		for col, v := range vals {
