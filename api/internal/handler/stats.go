...
//	@Param			zaehlpunkt	query	string	false	"Filter by Zählpunkt (substring match, extracted from the message XML at ingest time)"
...
func (h *StatsHandler) GetEDAMessages(w http.ResponseWriter, r *http.Request) {
	...
	// Optional server-side filters — keeps pagination consistent when filtering by type/direction.
	filterDirection := r.URL.Query().Get("direction")    // "inbound"|"outbound"|""
	filterProcess := r.URL.Query().Get("process")        // e.g. "EC_REQ_ONL"|""
	filterZaehlpunkt := r.URL.Query().Get("zaehlpunkt")  // substring match against xml_payload

	totalCount, err := h.edaMessageRepo.CountByEEG(r.Context(), eegID, filterDirection, filterProcess, filterZaehlpunkt)
	if err != nil {
		jsonError(w, "failed to count EDA messages", http.StatusInternalServerError)
		return
	}

	msgs, err := h.edaMessageRepo.ListByEEG(r.Context(), eegID, filterDirection, filterProcess, filterZaehlpunkt, limit, offset)
	if err != nil {
