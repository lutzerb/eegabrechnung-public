...
func (h *OnboardingHandler) UpdateOnboardingStatus(w http.ResponseWriter, r *http.Request) {
	...
	if body.Status == "approved" {
		...
		seenZP := make(map[string]bool)
		for _, mp := range req.MeterPoints {
			z := strings.TrimSpace(mp.Zaehlpunkt)
			if z == "" {
				continue
			}
			if seenZP[z] {
				jsonError(w, fmt.Sprintf("Zählpunkt %s ist mehrfach im Antrag eingetragen — bitte Antrag korrigieren", z), http.StatusBadRequest)
				return
			}
			seenZP[z] = true
		}
		...
		for _, mp := range req.MeterPoints {
			...
			if _, err := tx.Exec(ctx,
				`INSERT INTO meter_point_registration_periods (eeg_id, zaehlpunkt, meter_point_id, registriert_seit, source)
				 SELECT $1, $2, $3, $4, 'eda'
				 WHERE NOT EXISTS (
				   SELECT 1 FROM meter_point_registration_periods
				   WHERE eeg_id = $1 AND zaehlpunkt = $2 AND abgemeldet_am IS NULL
				 )`,
				eegID, mp.Zaehlpunkt, mpID, beitrittsDatum,
			); err != nil {
				slog.Error("failed to open registration period", "error", err, "zaehlpunkt", mp.Zaehlpunkt)
				jsonError(w, fmt.Sprintf("failed to create meter point %s: %v", mp.Zaehlpunkt, err), http.StatusInternalServerError)
				return
			}
...
<p style="color: #64748b; font-size: 14px;">Oder öffnen Sie: <a href="%s">%s</a></p>
	...
	for _, rcpt := range recipients {
		...
		if err := invoice.SendLogged(ctx, h.emailLogRepo, smtpCfg, eeg.ID, "onboarding_admin", rcpt.email, subject, nil, nil, []byte(msg.String())); err != nil {
...
<p style="color: #64748b; font-size: 14px;">Falls Sie keinen Beitrittsantrag gestellt haben, können Sie diese E-Mail ignorieren.</p>
	...
	if err := invoice.SendLogged(context.Background(), h.emailLogRepo, smtpCfg, eegID, "onboarding_verify", toEmail, subject, nil, nil, []byte(msgBuilder.String())); err != nil {
...
<p style="color: #64748b; font-size: 14px;">Bei Fragen wenden Sie sich bitte direkt an die Energiegemeinschaft.</p>
	...
	return invoice.SendLogged(context.Background(), h.emailLogRepo, smtpCfg, req.EegID, "onboarding_token", req.Email, subject, nil, nil, msgBytes)
}
...
<p style="color: #94a3b8; font-size: 12px;">Bei Fragen wenden Sie sich bitte direkt an die Energiegemeinschaft.</p>
	...
	return invoice.SendLogged(context.Background(), h.emailLogRepo, smtpCfg2, req.EegID, "onboarding_conversion", req.Email, subject, nil, nil, []byte(msgBuilder.String()))
}

// RunReminderCheck sends two kinds of 72-hour follow-up emails:
//  1. eda_sent requests where the member hasn't confirmed NB data access yet.
//  2. Abandoned email verifications: someone entered their email but never
...
func (h *OnboardingHandler) RunReminderCheck(ctx context.Context) {
	// 1. eda_sent reminders (existing logic)
	...
	// 2. Abandoned form reminders
	abandoned, err := h.onboardingRepo.FindAbandonedEmailVerifications(ctx, 72*time.Hour)
	if err != nil {
		slog.Error("reminder check: abandoned query failed", "error", err)
		return
	}
	for _, a := range abandoned {
		eeg, err := h.eegRepo.GetByIDInternal(ctx, a.EegID)
		if err != nil || eeg.SMTPHost == "" {
			continue
		}
		if err := h.sendAbandonedFormEmail(a, eeg); err != nil {
...
// sendReminderEmail sends the 72-hour follow-up reminder to an applicant.
// For eda_sent: asks the member to confirm data access in the NB portal.
// For pending:  informs the member their application is still under review.
// The EEG admin (eeg.SMTPFrom) is always CC'd so nothing falls through the cracks.
...
<p style="color: #94a3b8; font-size: 12px;">Diese Erinnerung wurde automatisch verschickt, da seit Ihrer Aufnahme noch keine Bestätigung der Datenfreigabe vorliegt.</p>
		...
		subject = fmt.Sprintf("Erinnerung: Ihr Beitrittsantrag bei %s wartet auf Bearbeitung", eegName)
		htmlBody = fmt.Sprintf(`<!DOCTYPE html>
<html>
<head><meta charset="utf-8"></head>
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1e293b;">
<h2 style="color: #1e40af;">Erinnerung: Beitrittsantrag in Bearbeitung</h2>
<p>Liebe(r) %s,</p>
<p>wir möchten Sie daran erinnern, dass Ihr Beitrittsantrag zur Energiegemeinschaft <strong>%s</strong> eingegangen ist und noch bearbeitet wird.</p>
<p>Sobald Ihr Antrag geprüft wurde, erhalten Sie eine weitere Nachricht von uns.</p>
<p style="color: #475569; font-size: 14px;">Bei Fragen wenden Sie sich bitte direkt an die Energiegemeinschaft.</p>
<p style="margin: 24px 0;">
  <a href="%s" style="background-color: #1e40af; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
    Antragsstatus ansehen
...
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<p style="color: #94a3b8; font-size: 12px;">Diese Erinnerung wurde automatisch verschickt, da Ihr Antrag seit mehr als 72 Stunden offen ist.</p>
</body>
</html>`, req.Name1, eegName, statusURL)
	}

	// Build recipients: member + admin CC (deduplicated).
	recipients := []string{req.Email}
	ccHeader := ""
	if adminEmail != "" && adminEmail != req.Email {
		recipients = append(recipients, adminEmail)
		ccHeader = "Cc: " + adminEmail + "\r\n"
	}
	...
	sendErr := smtp.SendMail(smtpCfg.Host, auth, smtpCfg.From, recipients, msgBytes)
	...
	h.emailLogRepo.Log(context.Background(), entry)
...
<head><meta charset="utf-8"></head>
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1e293b;">
<h2 style="color: #1e40af;">Beitrittsanmeldung nicht abgeschlossen</h2>
<p>Hallo %s,</p>
<p>Sie haben vor einigen Tagen begonnen, der Energiegemeinschaft <strong>%s</strong> beizutreten – wir haben Ihre E-Mail-Adresse erhalten, jedoch wurde der Beitrittsantrag nicht abgeschlossen.</p>
<p>Falls Sie noch Interesse haben, können Sie die Anmeldung jederzeit fortsetzen:</p>
<p style="margin: 24px 0;">
  <a href="%s" style="background-color: #1e40af; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
    Jetzt Beitrittsantrag ausfüllen
  </a>
</p>
<p style="color: #475569; font-size: 14px;">Falls Sie kein Interesse mehr haben, können Sie diese E-Mail einfach ignorieren.</p>
<p style="color: #475569; font-size: 14px;">Bei Fragen wenden Sie sich gerne direkt an die Energiegemeinschaft.</p>
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<p style="color: #94a3b8; font-size: 12px;">Diese Erinnerung wurde automatisch verschickt, da Ihre Anmeldung seit mehr als 3 Tagen offen ist.</p>
	...
	ccHeader := ""
	if adminEmail != "" && adminEmail != a.Email {
		recipients = append(recipients, adminEmail)
		ccHeader = "Cc: " + adminEmail + "\r\n"
	}
	...
	entry := domain.EmailLogEntry{
		EegID:     a.EegID,
		EmailType: "onboarding_abandoned_reminder",
		ToAddress: a.Email,
		Subject:   subject,
	}
	if sendErr != nil {
...
// manualOnboardingRequest is the body for POST /api/v1/eegs/{eegID}/onboarding/manual.
...
// CreateManualOnboarding handles POST /api/v1/eegs/{eegID}/onboarding/manual (auth required).
// Admin creates a pre-filled onboarding request on behalf of a member. The member receives
...
func (h *OnboardingHandler) CreateManualOnboarding(w http.ResponseWriter, r *http.Request) {
	...
	if emailExists, err := h.memberRepo.ExistsByEmailInEEG(r.Context(), eegID, strings.TrimSpace(body.Email)); err == nil && emailExists {
		jsonError(w, "Diese E-Mail-Adresse ist bereits als Mitglied registriert.", http.StatusConflict)
		return
	}
	if pending, err := h.onboardingRepo.FindPendingByEmailAndEEG(r.Context(), strings.TrimSpace(body.Email), eegID); err == nil && pending != nil {
		jsonError(w, "Es gibt bereits einen offenen Antrag für diese E-Mail-Adresse.", http.StatusConflict)
		return
	}
	for _, mp := range body.MeterPoints {
		if strings.TrimSpace(mp.Zaehlpunkt) == "" {
	...
	if err := h.onboardingRepo.Create(r.Context(), req); err != nil {
		slog.Error("failed to create manual onboarding request", "error", err)
		jsonError(w, "failed to create onboarding request", http.StatusInternalServerError)
		return
	}

	if !eeg.IsDemo {
		if err := h.sendManualConfirmationEmail(req, eeg); err != nil {
			slog.Warn("failed to send manual confirmation email", "error", err, "request_id", req.ID)
		}
	}
...
// ConfirmManualOnboarding handles POST /api/v1/public/onboarding/confirm/{token}.
// The member clicks the link from the confirmation email, reviews their data, and submits.
// Transitions the request from 'admin_created' → 'pending' and records the acceptance IP/time.
func (h *OnboardingHandler) ConfirmManualOnboarding(w http.ResponseWriter, r *http.Request) {
	token := chi.URLParam(r, "token")
	if token == "" {
		jsonError(w, "token is required", http.StatusBadRequest)
		return
	...
	if err != nil {
		if strings.Contains(err.Error(), "token expired") {
			jsonError(w, "Link abgelaufen", http.StatusGone)
			return
		}
		slog.Error("failed to get onboarding by token for confirm", "error", err)
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
	if req == nil {
	...
	if req.Status != "admin_created" {
		jsonError(w, "Dieser Link ist nicht mehr gültig.", http.StatusConflict)
		return
	}
	...
	if err := h.onboardingRepo.Confirm(r.Context(), req.ID, ip); err != nil {
		slog.Error("failed to confirm manual onboarding", "error", err)
		jsonError(w, "internal error", http.StatusInternalServerError)
		return
	}
...
// sendManualConfirmationEmail sends the admin-created confirmation email to the member.
// It includes a data summary, the AGB/contract text, SEPA mandate details, document links,
// and a magic-link button that takes the member to the read-only confirmation page.
func (h *OnboardingHandler) sendManualConfirmationEmail(req *domain.OnboardingRequest, eeg *domain.EEG) error {
	if eeg.SMTPHost == "" {
		slog.Info("SMTP not configured, skipping manual confirmation email", "request_id", req.ID)
		return nil
	}
	smtpCfg := invoice.SMTPConfig{Host: eeg.SMTPHost, From: eeg.SMTPFrom, Username: eeg.SMTPUser, Password: eeg.SMTPPassword}
	...
	var mpRows strings.Builder
	for _, mp := range req.MeterPoints {
		if strings.TrimSpace(mp.Zaehlpunkt) == "" {
			continue
		}
		dirLabel := "Bezug"
		if mp.Direction == "GENERATION" {
			dirLabel = "Einspeisung"
		}
		mpRows.WriteString(fmt.Sprintf(`<tr><td style="padding: 6px 12px 6px 0; color: #64748b; white-space: nowrap;"></td><td style="padding: 6px 0; font-family: monospace; font-size: 13px;">%s (%s)</td></tr>`, mp.Zaehlpunkt, dirLabel))
	}

	beitrittStr := "—"
	if req.BeitrittsDatum != nil {
		beitrittStr = req.BeitrittsDatum.Format("02.01.2006")
	}

	dataTable := fmt.Sprintf(`
<table style="border-collapse: collapse; width: 100%%; margin: 16px 0;">
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b; white-space: nowrap;">Name</td><td style="padding: 6px 0; font-weight: bold;">%s</td></tr>
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b;">Adresse</td><td style="padding: 6px 0;">%s, %s %s</td></tr>
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b;">E-Mail</td><td style="padding: 6px 0;">%s</td></tr>
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b;">IBAN</td><td style="padding: 6px 0; font-family: monospace;">%s</td></tr>
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b;">Typ</td><td style="padding: 6px 0;">%s</td></tr>
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b; vertical-align: top;">Zählpunkt(e)</td><td style="padding: 6px 0;">%s</td></tr>
  <tr><td style="padding: 6px 12px 6px 0; color: #64748b;">Beitritt</td><td style="padding: 6px 0;">%s</td></tr>
</table>`,
		fullName,
		...
		func() string {
			...
			for _, mp := range req.MeterPoints {
				if mp.Zaehlpunkt != "" {
					...
					zps = append(zps, fmt.Sprintf(`<span style="font-family:monospace;font-size:13px;">%s</span> <span style="color:#64748b;font-size:12px;">(%s)</span>`, mp.Zaehlpunkt, dir))
				}
			}
			return strings.Join(zps, "<br>")
		}(),
		beitrittStr,
	)

	// Contract / SEPA block
	var contractBlock string
	if eeg.OnboardingContractText != "" {
		contractBlock = fmt.Sprintf(`
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<h3 style="color: #1e293b;">Einzugsermächtigung / SEPA-Lastschrift</h3>
<div style="background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; color: #475569; font-size: 14px; line-height: 1.6; white-space: pre-wrap;">%s</div>`,
			eeg.OnboardingContractText)
	} else if eeg.SepaCreditorID != "" {
		contractBlock = fmt.Sprintf(`
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<h3 style="color: #1e293b;">SEPA-Lastschrift</h3>
<p style="color: #475569; font-size: 14px;">Mit Ihrer Bestätigung erteilen Sie der %s (Gläubiger-ID: %s) ein SEPA-Lastschriftmandat für Ihren Bankkontakt (IBAN: %s). Zahlungen werden gemäß den Abrechnungsbedingungen eingezogen.</p>`,
			eeg.Name, eeg.SepaCreditorID, req.IBAN)
	}
...
<head><meta charset="utf-8"></head>
<body style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; color: #1e293b;">
<h2 style="color: #1e40af;">Anmeldung bestätigen – %s</h2>
<p>Liebe(r) %s,</p>
<p>der Administrator der Energiegemeinschaft <strong>%s</strong> hat Ihre Anmeldung eingetragen. Bitte prüfen Sie die eingetragenen Daten und bestätigen Sie Ihre Anmeldung.</p>
<h3 style="color: #1e293b; margin-top: 24px;">Ihre eingetragenen Daten</h3>
%s
%s
%s
<hr style="border: none; border-top: 1px solid #e2e8f0; margin: 24px 0;">
<p style="color: #475569;">Wenn Ihre Daten korrekt sind, klicken Sie bitte auf den folgenden Button um Ihre Anmeldung zu bestätigen:</p>
<p style="margin: 24px 0;">
  <a href="%s" style="background-color: #16a34a; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; font-weight: bold;">
    Anmeldung bestätigen
  </a>
</p>
<p style="color: #64748b; font-size: 14px;">Oder kopieren Sie diesen Link: <a href="%s">%s</a></p>
<p style="color: #64748b; font-size: 14px;">Dieser Link ist 30 Tage gültig.</p>
<p style="color: #64748b; font-size: 14px;">Falls Ihre Daten nicht korrekt sind, wenden Sie sich bitte direkt an die Energiegemeinschaft – bestätigen Sie den Link in diesem Fall <strong>nicht</strong>.</p>
</body>
</html>`,
		eeg.Name,
		req.Name1, eeg.Name,
	...
	return invoice.SendLogged(context.Background(), h.emailLogRepo, smtpCfg, req.EegID, "onboarding_manual_confirm", req.Email, subject, nil, nil, []byte(msgBuilder.String()))
}
