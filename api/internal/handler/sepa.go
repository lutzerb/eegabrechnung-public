...
func (h *SEPAHandler) DownloadPain001(w http.ResponseWriter, r *http.Request) {
	...
	// Execution date = next workday after the pain.008 collection date,
	// so the account is funded before producers are paid.
	p008Entries := buildP008Entries(invoices, membersByID)
	collectionDate := sepa.Pain008CollectionDate(eeg, p008Entries)
	execDate := sepa.NextWorkday(collectionDate)

	xmlBytes, err := sepa.GeneratePain001(eeg, entries, execDate)
	if err != nil {
...
// ImportBillingCAMT053 godoc
//
//	@Summary		CAMT.053 Kontoauszug importieren und Rechnungen als bezahlt markieren
//	@Description	Liest alle TxDtls-EndToEndIds aus dem CAMT.053. Jede EndToEndId, die einer Rechnung entspricht (UUID ohne Bindestriche), wird als bezahlt markiert (status=paid, paid_at=Buchungsdatum).
//	@Tags			Billing
//	@Accept			multipart/form-data
//	@Produce		json
//	@Param			eegID	path		string	true	"EEG ID (UUID)"
//	@Param			file	formData	file	true	"CAMT.053 XML-Datei"
//	@Success		200		{object}	object	"matched, not_found, already_paid"
//	@Failure		400		{object}	map[string]string
//	@Security		BearerAuth
//	@Router			/eegs/{eegID}/billing/camt053 [post]
func (h *SEPAHandler) ImportBillingCAMT053(w http.ResponseWriter, r *http.Request) {
	claims := auth.ClaimsFromContext(r.Context())
	if claims == nil {
		jsonError(w, "unauthorized", http.StatusUnauthorized)
		return
	...
	// Extract per-sub-transaction EndToEndIds with their booking dates.
	// ParseCAMT053 collapses batch entries to one row; here we need each TxDtls separately.
	txMatches := extractCAMT053EndToEndIds(data)
	if txMatches == nil {
		jsonError(w, "failed to parse CAMT.053", http.StatusUnprocessableEntity)
		return
	}
	...
	for _, tx := range txMatches {
		...
		normalized := strings.ReplaceAll(tx.endToEndID, "-", "")
		if len(normalized) != 32 {
			notFound++
			continue
		}
		// Re-insert dashes to form a valid UUID string.
		withDashes := normalized[0:8] + "-" + normalized[8:12] + "-" + normalized[12:16] + "-" + normalized[16:20] + "-" + normalized[20:32]
		invoiceID, parseErr := uuid.Parse(withDashes)
		if parseErr != nil {
			notFound++
			continue
		}
		...
		inv, fetchErr := h.invoiceRepo.GetByID(r.Context(), invoiceID)
		if fetchErr != nil || inv.EegID != eegID {
			notFound++
			continue
		}
		if inv.Status == "paid" {
...
func extractCAMT053EndToEndIds(data []byte) []camt053TxMatch {
	...
	for _, stmt := range doc.BkToCstmrStmt.Stmt {
		for _, e := range stmt.Ntry {
			bookingDate, err := time.Parse("2006-01-02", strings.TrimSpace(e.BookgDt.Dt))
			if err != nil {
				continue
			}
