...
export interface EEG {
  ...
  eda_dis_model: string; // S (statisch) | D (dynamisch) — ECDisModel, für die ganze Gemeinschaft festgelegt
  // Accounting / DATEV
...
export interface Invoice {
  ...
  generation_vat_pct?: number;
  generation_vat_amount?: number;
  consumption_net_amount?: number;
  consumption_vat_amount?: number;
  pdf_path?: string;
...
/** Returns the actual cash transfer amount for an invoice.
 *  For Reverse Charge generation (§ 19 Abs. 1 UStG) the EEG remits the generation
 *  VAT to the Finanzamt — only the net flows to/from the member.
 *  Formula: total_amount + generation_vat_amount (= consumptionGross − generationNet).
 *  For Landwirt (13 %) and all non-RC cases: total_amount is unchanged. */
export function invoiceZahlBetrag(inv: Invoice): number {
  if ((inv.generation_vat_pct ?? 0) >= 20 && (inv.generation_vat_amount ?? 0) > 0) {
    return (inv.total_amount ?? 0) + (inv.generation_vat_amount ?? 0);
  }
...
export interface MeterPointRegistrationPeriod {
  id: string;
  eeg_id: string;
  zaehlpunkt: string;
  meter_point_id?: string;
  ...
  abgemeldet_am?: string;
  consent_id: string;
  source: string; // eda | manual | backfill
  reason: string;
  created_at: string;
}

export interface EDAProcessEvent {
  id: string;
  eeg_id: string;
  meter_point_id?: string;
  process_type: string; // EC_REQ_ONL | CM_REV_SP
  status: string; // pending | sent | first_confirmed | confirmed | completed | rejected | error
  zaehlpunkt: string;
  valid_from?: string;
  participation_factor?: number;
...
export interface MeterPointHistory {
...
export interface CreateMeterPointRequest {
  ...
  abgemeldet_am?: string;   // YYYY-MM-DD or "clear" to unset
...
export async function getMeterPointHistory(
  token: string,
  eegId: string,
  meterPointId: string
): Promise<MeterPointHistory> {
  return apiFetch<MeterPointHistory>(
    `/eegs/${eegId}/meter-points/${meterPointId}/history`,
    token
  );
}

export interface SepaMandateHistoryEntry {
  id: string;
  member_id: string;
  eeg_id: string;
  iban: string;
...
export async function getMemberSepaMandateHistory(
  token: string,
  eegId: string,
  memberId: string
): Promise<SepaMandateHistoryEntry[]> {
  return apiFetch<SepaMandateHistoryEntry[]>(
    `/eegs/${eegId}/members/${memberId}/sepa-mandat/history`,
    token
  );
}
...
export async function listEDAMessages(
  token: string,
  eegId: string,
  options?: { limit?: number; offset?: number; direction?: string; process?: string; zaehlpunkt?: string }
  ...
  if (options?.zaehlpunkt) params.set("zaehlpunkt", options.zaehlpunkt);
...
export interface MemberStat {
  ...
  total_amount: number;
  invoice_count: number;
  l3_reading_count: number;      // L3 (faulty) readings in period; 0 in billed mode
}
...
export interface TariffSchedule {
  id: string;
  eeg_id: string;
  member_id?: string; // set for member-specific Individualtarife, absent for the EEG default
  name: string;
  ...
  entries?: TariffEntry[];
  created_at: string;
  // Individualtarif overrides — null/undefined means "use EEG default"
