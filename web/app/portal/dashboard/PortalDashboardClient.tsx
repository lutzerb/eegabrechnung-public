...
export default function PortalDashboardClient({ member, eeg, invoices, documents, meterPoints, showFullEnergy }: Props) {
  ...
  const [ibanConfirm, setIbanConfirm] = useState(false);
  ...
  async function handleChangeIban() {
    if (!ibanConfirm) {
      setIbanError("Bitte bestätigen Sie das SEPA-Mandat mit der neuen IBAN.");
      return;
    }
    ...
    setIbanSuccess(null);
    try {
      const res = await fetch("/api/portal/sepa-mandate", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ iban: newIban.trim(), confirm: true }),
      });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) {
        setIbanError((data as { error?: string }).error || "Fehler beim Speichern der neuen IBAN.");
      } else {
        setCurrentIban((data as { iban?: string }).iban || newIban.trim());
        setIbanSuccess("Neue IBAN gespeichert und SEPA-Mandat bestätigt. Das bisherige Mandat wurde archiviert.");
        setEditingIban(false);
        setNewIban("");
        setIbanConfirm(false);
      }
    } catch {
          ...
          <div className="space-y-4">
            ...
            <div className="bg-white rounded-xl border border-slate-200 p-5">
              ...
              <p className="text-xs text-slate-500 mb-4">
                Aktuelle IBAN: <span className="font-mono">{currentIban ? maskIban(currentIban) : "nicht hinterlegt"}</span>
              </p>

              {!editingIban ? (
                <button
                  onClick={() => { setEditingIban(true); setNewIban(""); setIbanConfirm(false); setIbanError(null); setIbanSuccess(null); }}
                ...
                <div className="pt-1 border-t border-slate-100">
                  <p className="text-xs text-slate-500 mb-2 mt-3">Neue IBAN:</p>
                  <input
                    type="text"
                    value={newIban}
                    onChange={e => setNewIban(e.target.value)}
                    placeholder="AT12 3456 7890 1234 5678"
                    className="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm font-mono focus:outline-none focus:ring-2 focus:ring-blue-500"
                  />
                  <label className="flex items-start gap-2 mt-3 text-xs text-slate-600">
                    <input
                      type="checkbox"
                      checked={ibanConfirm}
                      onChange={e => setIbanConfirm(e.target.checked)}
                      className="mt-0.5"
                    />
                    <span>
                      Ich erteile der {eeg.name} hiermit ein neues SEPA-Lastschriftmandat für die oben angegebene IBAN.
                      Das bisherige Mandat wird archiviert und verliert seine Gültigkeit.
                    </span>
                  </label>
                  <div className="flex items-center gap-2 mt-3">
                    <button
                      onClick={handleChangeIban}
                      disabled={ibanSubmitting || !newIban.trim()}
                      className="px-3 py-1.5 bg-blue-600 text-white text-xs font-medium rounded-md hover:bg-blue-700 disabled:opacity-50 transition-colors"
                    >
                      {ibanSubmitting ? "Wird gespeichert…" : "Bestätigen"}
                    </button>
                    <button
                      onClick={() => { setEditingIban(false); setIbanError(null); }}
                      className="px-3 py-1.5 text-xs text-slate-500 hover:text-slate-700"
                    >
                      Abbrechen
                    </button>
