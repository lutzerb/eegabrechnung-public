...
export default function EmailComposer({ eegId }: Props) {
  ...
  const fileInputId = "comm-file-input";
  ...
  return (
    <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
      <div className="p-6 border-b border-slate-100">
        ...
        {/* Attachments */}
        <div className="mb-4">
          <p className="block text-sm font-medium text-slate-700 mb-1.5">Anhänge (optional)</p>
          <label htmlFor={fileInputId}
            className="inline-flex items-center gap-2 px-3 py-2 border border-dashed border-slate-300 rounded-lg text-sm text-slate-600 hover:border-blue-400 hover:text-blue-600 transition-colors cursor-pointer">
            <input id={fileInputId} type="file" multiple onChange={handleFileChange} className="sr-only" />
              ...
              {attachments.map((file, i) => (
                <li key={i} className="flex items-center justify-between gap-2 px-3 py-1.5 bg-slate-50 rounded-lg text-sm">
                  <button type="button"
                    ...
                    className="text-slate-700 truncate hover:text-blue-600 hover:underline text-left cursor-pointer">
                    {file.name}
                  </button>
          ...
          <div className="bg-white rounded-xl shadow-xl max-w-2xl w-full max-h-[80vh] flex flex-col">
            ...
            {attachments.length > 0 && (
              <div className="px-6 py-3 border-t border-slate-100 bg-slate-50">
                <p className="text-xs text-slate-500 mb-1.5">{attachments.length} Anhang/Anhänge:</p>
                <div className="flex flex-wrap gap-2">
                  {attachments.map((file, i) => (
                    <button key={i} type="button"
                      ...
                      className="inline-flex items-center gap-1.5 px-2 py-1 bg-white border border-slate-200 rounded text-xs text-blue-600 hover:bg-blue-50 hover:border-blue-300 transition-colors cursor-pointer">
