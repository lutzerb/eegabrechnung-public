...
import type { ForecastDayTotal } from "@/components/energy-charts";
import type {
  EnergySummaryRow,
  MonthlyEnergyRow,
  MemberStat,
...
export default function ReportsPage() {
  ...
  // Member + meter point filter
  const [selectedMemberId, setSelectedMemberId] = useState<string>("");
  const [selectedZaehlpunkt, setSelectedZaehlpunkt] = useState<string>("");
  ...
  // Forecast (auto-loaded for month view when current/future month is selected)
  const [forecastDailyTotals, setForecastDailyTotals] = useState<ForecastDayTotal[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (dataSource === "raw") {
        ...
        const zpParam = selectedZaehlpunkt ? `&zaehlpunkt=${encodeURIComponent(selectedZaehlpunkt)}` : "";
        const [summaryRes, memberStatsRes] = await Promise.all([
          fetch(
            `/api/eegs/${eegId}/energy/summary?from=${from}&to=${to}&granularity=${granularity}${memberParam}${zpParam}`,
            { headers: authHeaders }
          ),
          // Raw mode: use actual readings per member (works for any date range incl. single day)
          fetch(`/api/eegs/${eegId}/energy/members?from=${from}&to=${to}${memberParam}${zpParam}`, {
            headers: authHeaders,
          }),
        ]);
  ...
  }, [session, eegId, dataSource, getRange, periodMode, selectedYear, selectedMemberId, selectedZaehlpunkt]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    load();
  }, [load]);

  // Auto-fetch forecast when viewing current or future month in raw mode.
  useEffect(() => {
    if (!session?.accessToken || dataSource !== "raw" || periodMode !== "month") {
      setForecastDailyTotals([]);
      return;
    }
    const now = new Date();
    ...
    let cancelled = false;
    fetch(`/api/eegs/${eegId}/forecast`, { headers: authHeaders })
      .then((r) => r.json())
      .then((data) => {
        if (cancelled || data.status !== "ok") return;
        // Aggregate hourly entries to daily totals.
        const byDay = new Map<string, { c: number; g: number }>();
        for (const e of data.entries ?? []) {
          const day: string = (e.ts as string).slice(0, 10);
          const acc = byDay.get(day) ?? { c: 0, g: 0 };
          byDay.set(day, {
            c: acc.c + (e.consumption_kwh ?? 0),
            g: acc.g + (e.generation_kwh ?? 0),
          });
        }
        setForecastDailyTotals(
          [...byDay.entries()]
            .sort(([a], [b]) => a.localeCompare(b))
            .map(([day, v]) => ({ day, consumption_kwh: v.c, generation_kwh: v.g }))
        );
      })
      .catch(() => { /* forecast unavailable — silently skip */ });
    return () => { cancelled = true; };
  }, [session, eegId, dataSource, periodMode, selectedMonth]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-4 mb-6 flex flex-wrap gap-4 items-end">
        ...
        {members.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <MemberCombobox
              ...
              onChange={(id) => { setSelectedMemberId(id); setSelectedZaehlpunkt(""); }}
              className="w-56"
            />
            {selectedMemberId && (
              <button
                onClick={() => { setSelectedMemberId(""); setSelectedZaehlpunkt(""); }}
        ...
        {/* Meter point filter — only shown when a member with multiple ZPs is selected */}
        {(() => {
          const mp = members.find((m) => m.id === selectedMemberId)?.meter_points ?? [];
          if (!selectedMemberId || mp.length < 2) return null;
          return (
            <div className="flex items-center gap-2">
              <label className="text-xs text-slate-500 whitespace-nowrap">Zählpunkt:</label>
              <select
                value={selectedZaehlpunkt}
                onChange={(e) => setSelectedZaehlpunkt(e.target.value)}
                className="px-3 py-1.5 text-sm border border-slate-200 rounded-lg bg-white text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
              >
                <option value="">Alle ZP</option>
                {mp.map((p) => (
                  <option key={p.id} value={p.meter_id}>
                    {p.meter_id} ({p.direction === "CONSUMPTION" ? "Bezug" : "Einspeisung"})
                  </option>
                ))}
              </select>
              {selectedZaehlpunkt && (
                <button
                  onClick={() => setSelectedZaehlpunkt("")}
                  className="text-xs text-slate-400 hover:text-slate-600"
                  title="ZP-Filter zurücksetzen"
                >
                  ✕
                </button>
              )}
      ...
      {/* L3 quality alert */}
      {dataSource === "raw" && (() => {
        const l3Members = displayedMemberStats.filter((s) => s.l3_reading_count > 0);
        if (l3Members.length === 0) return null;
        const names = l3Members
          .map((s) => memberNames[s.member_id] || s.member_id.slice(0, 8))
          .join(", ");
        return (
          <div className="mb-6 flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl text-sm text-amber-800">
            ...
            <span>
              <strong>L3-Messwerte im Zeitraum:</strong> Der Netzbetreiber hat fehlerhafte Messwerte (Qualitätskennzeichen L3) übermittelt — diese werden nicht abgerechnet.
              {" "}Betroffen: {names}.
            </span>
          </div>
        );
                    ...
                    <tr key={s.member_id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-4 py-2.5 font-medium text-slate-800">
                        <span className="flex items-center gap-1.5">
                          {memberNames[s.member_id] || s.member_id.slice(0, 8)}
                          {s.l3_reading_count > 0 && (
                            <span
                              title={`${s.l3_reading_count} L3-Messwert${s.l3_reading_count !== 1 ? "e" : ""} (fehlerhaft, nicht abgerechnet)`}
