...
export default async function MemberDetailPage({ params }: Props) {
  ...
  // Active Individualtarif for this member, if any
  let activeIndividualtarif: Awaited<ReturnType<typeof listMemberTariffs>>[number] | null = null;
  try {
    const memberTariffs = await listMemberTariffs(session.accessToken!, eegId, memberId);
    activeIndividualtarif = memberTariffs.find((t) => t.is_active) || null;
  } catch {
    // non-fatal
  }

  // Archived SEPA mandate history (past IBANs), for audit purposes
  let mandateHistory: Awaited<ReturnType<typeof getMemberSepaMandateHistory>> = [];
  try {
    mandateHistory = await getMemberSepaMandateHistory(session.accessToken!, eegId, memberId);
  } catch {
    // non-fatal
  }
  ...
  return (
    <div className="p-8 space-y-6">
      ...
      {/* SEPA-Mandats-Historie (nur bei mind. einer archivierten IBAN-Änderung) */}
      {mandateHistory.length > 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-6">
          <h2 className="text-base font-semibold text-slate-900 mb-1">Mandats-Historie</h2>
          <p className="text-xs text-slate-500 mb-4">
            Archivierte SEPA-Mandate aus früheren IBAN-Änderungen (zu Nachweiszwecken aufbewahrt).
          </p>
          <div className="space-y-3">
            {mandateHistory.map((entry) => (
              <div key={entry.id} className="flex items-start gap-3 pl-3 border-l-2 border-slate-200">
                <div className="flex-1">
                  <p className="text-sm font-mono text-slate-900">{entry.iban ? formatIBAN(entry.iban) : "—"}</p>
                  <p className="text-xs text-slate-500 mt-0.5">
                    Archiviert am {formatDate(entry.archived_at)}
                    {entry.reason === "iban_change_admin" && " · manuell durch Admin geändert"}
                    {entry.reason === "iban_change_portal" && " · vom Mitglied über das Portal geändert"}
                  </p>
                  {entry.signed_at && (
                    <p className="text-xs text-slate-400 mt-0.5">
                      Mandat unterzeichnet am {formatDate(entry.signed_at)}
                      {entry.signed_ip && ` · IP ${entry.signed_ip}`}
                    </p>
                  )}
                </div>
              </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 p-6">
        <div className="flex items-center justify-between mb-3">
          <h2 className="text-base font-semibold text-slate-900">Tarifplan</h2>
          <Link
            href={`/eegs/${eegId}/members/${memberId}/tariff`}
            className="px-3 py-1.5 text-xs font-medium text-white bg-amber-700 rounded-lg hover:bg-amber-800 transition-colors"
          >
            Individualtarif verwalten
          </Link>
        </div>
        {activeIndividualtarif ? (
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-sm font-medium text-slate-900">{activeIndividualtarif.name}</span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-xs">
              {([
                ["free_kwh_override", "Freikontingent", "kWh"],
                ["discount_pct_override", "Rabatt", "%"],
                ...
                return (
                  <div key={key}>
                    ...
                    {val !== null && val !== undefined ? (
                      <p className="font-medium text-amber-700">{val} {unit}</p>
                    ) : (
                      <p className="text-slate-400">EEG-Standard</p>
                    )}
                  </div>
                );
              })}
                  ...
                  <tr key={mp.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-3 text-xs">
                      <MeterPointStatusBadge
                        ...
                        abmeldungStatus={mp.abmeldung_status}
                      />
                    </td>
                    <td className="px-6 py-3 text-slate-600 text-xs">
                      {mp.registriert_seit
