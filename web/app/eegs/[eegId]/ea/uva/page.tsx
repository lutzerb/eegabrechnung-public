...
function periodLabel(u: UVAPeriode): string {
  if (u.periode_label) return u.periode_label;
  if (u.periodentyp === "QUARTAL" && u.periode_nr) return `Q${u.periode_nr}/${u.jahr}`;
  if (u.periodentyp === "MONAT" && u.periode_nr) {
    const months = ["", "Jän", "Feb", "Mär", "Apr", "Mai", "Jun", "Jul", "Aug", "Sep", "Okt", "Nov", "Dez"];
    return `${months[u.periode_nr]}/${u.jahr}`;
  }
  return `${u.jahr}`;
}
...
export default function UVAPage() {
  ...
  async function handleCreate() {
    setCreating(true);
    const periodeNr = newForm.periodentyp === "QUARTAL" ? newForm.quartal : newForm.monat;
    const body = { jahr: newForm.jahr, periodentyp: newForm.periodentyp, periode_nr: periodeNr };
  ...
  async function handleDelete(id: string, label: string) {
    if (!confirm(`UVA-Periode "${label}" wirklich löschen?`)) return;
    setDeleting(true);
    const res = await fetch(`/api/eegs/${eegId}/ea/uva/${id}`, { method: "DELETE" });
    if (res.ok || res.status === 204) {
      if (selectedId === id) { setSelectedId(null); setKennzahlen(null); }
      await load();
    } else {
              ...
              {uvas.map((u) => (
                <li key={u.id} className="flex items-stretch">
                  <button
                    ...
                    className={`flex-1 text-left px-4 py-3 hover:bg-slate-50 ${selectedId === u.id ? "bg-blue-50" : ""}`}
                  >
                  ...
                  <button
                    onClick={() => handleDelete(u.id, periodLabel(u))}
                    disabled={deleting}
                    title="Löschen"
                    className="px-3 text-slate-400 hover:text-red-600 hover:bg-red-50 disabled:opacity-50 border-l border-slate-100"
                  >
                    ×
                  </button>
                </li>
