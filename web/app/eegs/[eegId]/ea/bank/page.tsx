...
interface ImportResult {
  ...
  skipped: number;
  total: number;
  files: { name: string; imported: number; skipped: number; total: number }[];
  errors: string[];
}
...
export default function BankPage() {
  ...
  async function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const aggregate: ImportResult = { imported: 0, skipped: 0, total: 0, files: [], errors: [] };
    try {
      for (let idx = 0; idx < files.length; idx++) {
        const file = files[idx];
        if (files.length > 1) setUploadProgress(`(${idx + 1}/${files.length}) ${file.name}`);
        ...
        if (res.ok) {
          ...
          aggregate.total += r.total ?? 0;
          aggregate.files.push({ name: file.name, imported: r.imported ?? 0, skipped: r.skipped ?? 0, total: r.total ?? 0 });
        } else {
          const err = await res.json().catch(() => ({ error: "Unbekannter Fehler" }));
          aggregate.errors.push(`${file.name}: ${err.error || "Importfehler"}`);
        }
      }
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2">
          <label className={`px-4 py-2 text-sm font-medium text-white rounded-lg cursor-pointer ${uploading ? "bg-blue-400" : "bg-blue-700 hover:bg-blue-800"}`}>
            {uploading ? (uploadProgress || "Lädt hoch…") : "Kontoauszug importieren"}
            <input ref={fileRef} type="file" accept=".sta,.xml,.csv" multiple className="hidden" onChange={handleFileUpload} disabled={uploading} />
          </label>
        </div>
      </div>
      ...
      {importResult && (
        <div className="mb-4 p-4 bg-green-50 border border-green-200 rounded-lg">
          <p className="font-medium text-green-900 text-sm">
            Import abgeschlossen — {importResult.imported} neu, {importResult.skipped} bereits vorhanden übersprungen
            {importResult.files.length > 1 && ` (${importResult.files.length} Dateien)`}
          </p>
          {importResult.files.length > 1 && (
            <ul className="mt-2 space-y-0.5">
              {importResult.files.map((f, i) => (
                <li key={i} className="text-xs text-green-800 flex gap-2">
                  <span className="font-mono text-green-700 truncate max-w-xs">{f.name}</span>
                  <span>{f.imported} neu{f.skipped > 0 ? `, ${f.skipped} übersprungen` : ""}</span>
                </li>
              ))}
            </ul>
          )}
