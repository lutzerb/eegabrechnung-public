...
import OnboardingManualCreateButton from "./OnboardingManualCreateButton";
...
function StatusBadge({ status }: { status: string }) {
  const map: Record<string, { label: string; cls: string }> = {
    admin_created: { label: "Bestätigung ausstehend", cls: "bg-orange-100 text-orange-800" },
...
const STATUS_FILTERS = [
  { value: "", label: "Aktive" },
  { value: "admin_created", label: "Bestätigung ausstehend" },
...
export default async function OnboardingAdminPage({ params, searchParams }: Props) {
  ...
  const filtered =
    statusFilter === "alle"
      ...
      : requests.filter((r) => r.status !== "rejected" && r.status !== "converted" && r.status !== "active"); // default: open items only
