...
export default function OnboardingManualCreateModal({ eegId, onClose, onSuccess }: Props) {
  ...
  const [memberType, setMemberType] = useState("CONSUMER");
  const [businessRole, setBusinessRole] = useState("privat");
  const [uidNummer, setUidNummer] = useState("");
  const [useVat, setUseVat] = useState(false);
  const [beitrittsDatum, setBeitrittsDatum] = useState("");
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const res = await fetch(`/api/eegs/${eegId}/onboarding/manual`, {
        method: "POST",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl max-h-[90vh] flex flex-col">
        ...
        <div className="p-6 border-b border-slate-200 flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-xl font-bold text-slate-900">Mitglied manuell anlegen</h2>
            <p className="text-sm text-slate-500 mt-0.5">
              Das Mitglied erhält eine E-Mail mit den eingetragenen Daten und einem Bestätigungslink.
            </p>
          </div>
          <button
            type="button"
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="flex items-center gap-2 pt-5">
                <input
                  ...
                  className="rounded"
                />
                <label htmlFor="use_vat_create" className="text-sm text-slate-700">USt-pflichtig</label>
              </div>
              <div>
        ...
        <div className="p-6 border-t border-slate-200 flex gap-3 justify-end shrink-0">
          ...
          >
            {loading ? "Wird angelegt..." : "Anlegen & E-Mail senden"}
          </button>
        </div>
      </div>
    </div>
