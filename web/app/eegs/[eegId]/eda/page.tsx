...
interface Props {
  ...
  searchParams: Promise<{ tab?: string; page?: string; pageSize?: string; direction?: string; process?: string; zaehlpunkt?: string }>;
}
...
export default async function EDAPage({ params, searchParams }: Props) {
  ...
  const { tab, page, pageSize, direction: directionParam, process: processParam, zaehlpunkt: zaehlpunktParam } = await searchParams;
  const messageDirection = directionParam || "";
  const messageProcess = processParam || "";
  const messageZaehlpunkt = zaehlpunktParam || "";
  ...
  try {
    const [eegResult, messageResult, processResult, errorResult, workerStatusResult] = await Promise.all([
      ...
      listEDAMessages(session.accessToken!, eegId, {
        ...
        zaehlpunkt: messageZaehlpunkt || undefined,
