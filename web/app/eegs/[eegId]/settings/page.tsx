...
export default async function EEGSettingsPage({ params, searchParams }: Props) {
  ...
  async function saveSettings(formData: FormData) {
    ...
    const data: UpdateEEGRequest = {
      ...
      eda_dis_model: (formData.get("eda_dis_model") as string) || "D",
          ...
          <>
            <div className="bg-white rounded-xl border border-slate-200 p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className={labelClass}>Fixgebühr (EUR/Periode)</label>
                  <div className="relative">
              ...
              <div className="space-y-3">
                <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-200 cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                  <input
                    ...
                    defaultChecked={!eeg.invoice_payment_notice_mode || eeg.invoice_payment_notice_mode === "sepa_lastschrift"}
                    className="mt-0.5 h-4 w-4 border-slate-300 text-blue-700 focus:ring-blue-500"
                  />
                  <div>
                    <p className="text-sm font-medium text-slate-800">SEPA-Lastschrift</p>
                    <p className="text-xs text-slate-500 mt-0.5">Rechnung weist darauf hin, dass der Betrag automatisch per Lastschrift eingezogen wird (Standard).</p>
                  </div>
                </label>
                <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-200 cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                  <input
                    ...
                    name="invoice_payment_notice_mode"
                    value="ueberweisung"
                    defaultChecked={eeg.invoice_payment_notice_mode === "ueberweisung"}
                    className="mt-0.5 h-4 w-4 border-slate-300 text-blue-700 focus:ring-blue-500"
                  />
                  <div>
                    <p className="text-sm font-medium text-slate-800">Überweisungshinweis</p>
                    <p className="text-xs text-slate-500 mt-0.5">Rechnung zeigt stattdessen die IBAN (und BIC) der Energiegemeinschaft für eine manuelle Überweisung durch das Mitglied.</p>
                  </div>
                </label>
                <label className="flex items-start gap-3 p-3 rounded-lg border border-slate-200 cursor-pointer has-[:checked]:border-blue-500 has-[:checked]:bg-blue-50">
                  <input
                  ...
                  />
                  <div>
                    <p className="text-sm font-medium text-slate-800">Kein Zahlungshinweis</p>
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select name="eda_dis_model" defaultValue={eeg.eda_dis_model || "D"} className={inputClass}>
                  <option value="D">D — Dynamisch</option>
                  <option value="S">S — Statisch</option>
                </select>
                <p className="text-xs text-slate-400 mt-1">
                  Wird vorab für die gesamte Gemeinschaft festgelegt und in Anmeldung sowie Teilnahmefaktor-Änderungen an den Netzbetreiber übermittelt.
                </p>
              </div>
            </div>
          </div>
      ...
      {activeTab === "system" && (
        <>
          ...
          <div className="mt-8 p-6 bg-white rounded-xl border border-slate-200">
            <h3 className="text-base font-semibold text-slate-900 mb-1">E-Mail-Protokoll</h3>
            <p className="text-sm text-slate-500 mb-4">
              Liste aller versendeten und fehlgeschlagenen E-Mails (Rechnungen, Kampagnen, EDA-Benachrichtigungen, Onboarding …)
            </p>
            <Link
              href={`/eegs/${eegId}/settings/email-log`}
