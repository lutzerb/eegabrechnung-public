...
import SendRunButton from "@/components/send-run-button";
...
function viennaDateParts(date: Date): { year: number; month: number; day: number } {
...
function periodLabel(run: BillingRun): string {
  ...
  const sp = viennaDateParts(s);
  const ep = viennaDateParts(e);
  const sameMonth = sp.year === ep.year && sp.month === ep.month;
  if (sameMonth) {
    const lastOfMonth = new Date(ep.year, ep.month + 1, 0).getDate();
    if (sp.day === 1 && ep.day === lastOfMonth) {
      return s.toLocaleDateString("de-AT", { month: "long", year: "numeric", timeZone: VIENNA_TZ });
    }
  }
...
function isFullMonth(run: BillingRun): boolean {
  const sp = viennaDateParts(new Date(run.period_start));
  const ep = viennaDateParts(new Date(run.period_end));
  const lastOfMonth = new Date(ep.year, ep.month + 1, 0).getDate();
  return sp.year === ep.year && sp.month === ep.month && sp.day === 1 && ep.day === lastOfMonth;
}
...
function RunSection({
  ...
  const total = invoices.reduce((s, i) => s + invoiceZahlBetrag(i), 0);
  const ausgetauschteEnergie = invoices.reduce((s, i) => s + (i.consumption_kwh ?? 0), 0);
  const umsatz = invoices.reduce((s, i) => s + (i.consumption_net_amount ?? 0), 0);
  const abzulieferndeUSt = invoices.reduce((s, i) =>
    s + (i.consumption_vat_amount ?? 0) + ((i.generation_vat_pct ?? 0) >= 20 ? (i.generation_vat_amount ?? 0) : 0), 0);
  const fmtKwh = (kwh: number) => kwh >= 1000
    ? `${(kwh / 1000).toLocaleString("de-AT", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} MWh`
    : `${kwh.toLocaleString("de-AT", { minimumFractionDigits: 2, maximumFractionDigits: 2 })} kWh`;
    ...
    <details className="bg-white rounded-xl border border-slate-200 overflow-hidden" open>
        ...
        <div className="flex flex-col gap-1">
          ...
          {invoices.length > 0 && (
            <div className="flex items-center gap-4 text-xs text-slate-500">
              <span>⚡ {fmtKwh(ausgetauschteEnergie)}</span>
              <span>Umsatz: {formatCurrency(umsatz)}</span>
              {abzulieferndeUSt > 0.005 && (
                <span>USt. abzuliefern: {formatCurrency(abzulieferndeUSt)}</span>
              )}
            </div>
          )}
        </div>
      ...
      <div className="flex items-center gap-2 flex-wrap">
        ...
        {run.status === "finalized" && (
          <SendRunButton runId={run.id} sendRunAction={sendRunAction} />
        )}
        {run.status === "finalized" && (
            ...
            {invoices.map((invoice) => (
              <tr key={invoice.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-5 py-3 text-right font-medium text-slate-900">
                  {formatCurrency(invoiceZahlBetrag(invoice))}
                </td>
              ...
              <>
                <div className="flex items-center justify-between mb-4 flex-wrap gap-2">
                  ...
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="flex items-center gap-2">
                      <span className="text-xs text-slate-500">Bezahlt markieren:</span>
                      <BillingCamt053Upload eegId={eegId} />
                    </div>
