...
export default function OnboardingForm({
  ...
  function validateStep3(): boolean {
    ...
    const zaehlpunktSet = new Set<string>();
    filledPoints.forEach((mp, i) => {
      const z = mp.zaehlpunkt.trim().toUpperCase();
      if (zaehlpunktSet.has(z)) {
        const origIdx = formData.meterPoints.findIndex((m, fi) => fi !== i && m.zaehlpunkt.trim().toUpperCase() === z);
        errors[`zaehlpunkt_${i}`] = "Dieser Zählpunkt wurde bereits eingetragen.";
        if (origIdx >= 0) errors[`zaehlpunkt_${origIdx}`] = "Dieser Zählpunkt wurde bereits eingetragen.";
      } else {
        zaehlpunktSet.add(z);
      }
    });
