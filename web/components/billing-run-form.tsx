...
import { type BillingRun, type Member } from "@/lib/api";

type PreviewInvoice = { total_amount: number; generation_vat_pct?: number; generation_vat_amount?: number; consumption_kwh: number; generation_kwh: number; member_id: string };
const previewZahlBetrag = (inv: PreviewInvoice) =>
  (inv.generation_vat_pct ?? 0) >= 20 && (inv.generation_vat_amount ?? 0) > 0
    ? inv.total_amount + (inv.generation_vat_amount ?? 0)
    : inv.total_amount;
...
interface GapDetail {
  zaehlpunkt: string;
  missing_days: string[]; // YYYY-MM-DD
}
...
export default function BillingRunForm({ eegId, members }: BillingRunFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent, forceOverride?: boolean, preview = false) => {
    ...
    try {
      ...
      if (res.status === 422) {
        const data = await res.json();
        setDataGap({ details: data.details ?? [] });
        return;
      }
    ...
    <div className="bg-white rounded-xl border border-slate-200 p-6">
            ...
            <tbody>
              {previewResult.invoices.map((inv, i) => (
                <tr key={i} className="border-b border-blue-100">
                  ...
                  <td className="py-1 text-right font-medium">{previewZahlBetrag(inv).toFixed(2)} €</td>
                </tr>
              ))}
            </tbody>
            <tfoot>
              <tr className="border-t border-blue-300 font-semibold">
                ...
                <td className="py-1 text-right">
                  {previewResult.invoices.reduce((s, inv) => s + previewZahlBetrag(inv), 0).toFixed(2)} €
                </td>
              </tr>
            </tfoot>
          </table>
      ...
      {dataGap && (
        <div className="mb-5 p-4 bg-red-50 border border-red-200 rounded-lg">
          <p className="font-medium text-red-800">Unvollständige Messdaten</p>
          ...
          <p className="text-xs text-red-600 mt-3">
            Bitte zuerst die fehlenden Messdaten importieren. Mit &quot;Trotzdem abrechnen&quot; werden vorhandene Werte verwendet — fehlende Intervalle zählen als 0 kWh.
          </p>
          <button
            onClick={(e) => { e.preventDefault(); setDataGap(null); handleSubmit(new Event("submit") as unknown as React.FormEvent, true); }}
            className="mt-3 px-4 py-1.5 text-sm font-medium text-white bg-red-600 rounded hover:bg-red-700 transition-colors"
          >
            Trotzdem abrechnen (Datenlücken ignorieren)
          </button>
        </div>
      )}
