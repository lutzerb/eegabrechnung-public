...
export function EDAMessagesTable({ messages, eegId, totalCount, page, pageSize, activeDirection = "", activeProcess = "", activeZaehlpunkt = "" }: Props) {
  ...
  const buildHref = (
    ...
    nextZaehlpunkt = activeZaehlpunkt
  ) => {
    const params = new URLSearchParams(searchParams.toString());
    params.set("tab", "nachrichten");
    params.set("page", String(nextPage));
    params.set("pageSize", String(nextPageSize));
    if (nextDirection) params.set("direction", nextDirection); else params.delete("direction");
    if (nextProcess) params.set("process", nextProcess); else params.delete("process");
    if (nextZaehlpunkt) params.set("zaehlpunkt", nextZaehlpunkt); else params.delete("zaehlpunkt");
    return `${pathname}?${params.toString()}`;
  };

  // The search box matches against the raw XML of each message (server-side, across all
  // pages) — that's the only place a Zählpunkt actually appears, so plain client-side
  // filtering of the loaded page can't find it. Debounce to avoid a request per keystroke.
  useEffect(() => {
    if (search === activeZaehlpunkt) return;
    debounceRef.current = setTimeout(() => {
      window.location.href = buildHref(1, pageSize, activeDirection, activeProcess, search.trim());
    }, 500);
    return () => {
  ...
  // Only show the hard-empty state when there are truly no messages at all (no filters active).
  if (messages.length === 0 && !activeDirection && !activeProcess && !activeZaehlpunkt) {
    return (
  ...
  return (
    <>
      ...
      <div className="px-4 py-3 border-b border-slate-100 flex flex-wrap gap-3 items-center bg-slate-50/50">
        <input
          type="search"
          placeholder="Zählpunkt eingeben…"
          value={search}
        ...
        <select
          ...
          onChange={(e) => { window.location.href = buildHref(1, pageSize, e.target.value, activeProcess); }}
        ...
        <select
          value={activeProcess}
          onChange={(e) => { window.location.href = buildHref(1, pageSize, activeDirection, e.target.value); }}
          className="text-sm border border-slate-200 rounded-lg px-3 py-1.5 focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white"
        >
        ...
        {(search || activeDirection || activeProcess) && (
          <button
            onClick={() => {
              setSearch("");
              window.location.href = buildHref(1, pageSize, "", "", "");
            }}
