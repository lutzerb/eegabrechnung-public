...
| 118_tariff_entries_vienna_boundaries | Data-only fix: reinterprets every `tariff_entries.valid_from/valid_until` (annual/monthly schedules) from literal UTC calendar midnight to true Vienna-local midnight. The frontend built these as `` `${year}-${month}-01T00:00:00Z` `` while billing periods are computed Vienna-local (`viennaLoc` in `billing.go`) — the 1-2h (DST) mismatch caused `SumTOUForMember` to price the first hour(s) of every month/year with the *previous* tariff entry. `web/components/tariff-manager.tsx` now builds/matches entries via `viennaMidnightISO`/`viennaYearMonth` (Vienna-local, DST-aware). Not backfilled onto already-issued invoices — fixes future billing runs only |
